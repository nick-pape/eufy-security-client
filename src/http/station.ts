...
import {
  ...
  CommandName,
  StationCommands,
  StationGuardModeKeyPadProperty,
  StationCurrentModeKeyPadProperty,
  StationAutoEndAlarmProperty,
  StationSwitchModeWithAccessCodeProperty,
  StationTurnOffAlarmWithButtonProperty,
  PublicKeyType,
  MotionDetectionMode,
  VideoTypeStoreToNAS,
  HB3DetectionTypes,
  WalllightNotificationType,
  DailyLightingType,
...
import {
  ...
  getBlocklist,
  getFloodLightT8425Notification,
  getHB3DetectionMode,
...
import {
  ...
  LockAdvancedOnOffRequestPayload,
  AdvancedLockSetParamsType,
  PropertyData,
  CustomData,
  CommandData,
  StorageInfoBodyHB3,
...
import {
  ...
  getLockVectorBytes,
  isPrivateIp,
  getSmartSafeP2PCommand,
  getLockV12P2PCommand,
  getLockP2PCommand,
  RGBColorToDecimal,
  getSmartLockP2PCommand,
} from "../p2p/utils";
import {
  InvalidCommandValueError,
  InvalidPropertyValueError,
  NotSupportedError,
  RTSPPropertyNotEnabledError,
  WrongStationError,
  StationConnectTimeoutError,
  PinNotVerifiedError,
  ensureError,
} from "../error";
...
export class Station extends TypedEmitter<StationEvents> {
  ...
  protected constructor(
    ...
    this.p2pSession.on(
      "download started",
      (channel: number, metadata: StreamMetadata, videoStream: Readable, audioStream: Readable) =>
        this.onStartDownload(channel, metadata, videoStream, audioStream)
    );
    this.p2pSession.on("download finished", (channel: number) => this.onFinishDownload(channel));
    this.p2pSession.on(
      "livestream started",
      (channel: number, metadata: StreamMetadata, videoStream: Readable, audioStream: Readable) =>
        this.onStartLivestream(channel, metadata, videoStream, audioStream)
    );
    ...
    this.p2pSession.on("parameter", (channel: number, param: number, value: string) =>
      this.onParameter(channel, param, value)
    );
    this.p2pSession.on("runtime state", (channel: number, batteryLevel: number, temperature: number) =>
      this.onRuntimeState(channel, batteryLevel, temperature)
    );
    this.p2pSession.on("charging state", (channel: number, chargeType: number, batteryLevel: number) =>
    ...
    this.p2pSession.on("talkback started", (channel: number, talkbackStream: TalkbackStream) =>
      this.onTalkbackStarted(channel, talkbackStream)
    );
    ...
    this.p2pSession.on("shake alarm", (channel: number, event: SmartSafeShakeAlarmEvent) =>
      this.onDeviceShakeAlarm(channel, event)
    );
    this.p2pSession.on("911 alarm", (channel: number, event: SmartSafeAlarm911Event) =>
      this.onDevice911Alarm(channel, event)
    );
    ...
    this.p2pSession.on("garage door status", (channel: number, doorId: number, status: number) =>
      this.onGarageDoorStatus(channel, doorId, status)
    );
    this.p2pSession.on("storage info hb3", (channel: number, storageInfo: StorageInfoBodyHB3) =>
      this.onStorageInfoHB3(channel, storageInfo)
    );
  ...
  static async getInstance(
    api: HTTPApi,
    stationData: StationListResponse,
    ipAddress?: string,
    ...
    if (
      Device.isLock(stationData.device_type) &&
      ...
      !Device.isLockWifiT85D0(stationData.device_type)
    ) {
  ...
  public getStateID(state: string, level = 2): string {
    switch (level) {
      ...
      default:
        if (state) return `${this.getSerial()}.${this.getStateChannel()}.${state}`;
  ...
  public update(station: StationListResponse): void {
    ...
    for (const property of Object.values(metadata)) {
      if (this.rawStation[property.key] !== undefined && typeof property.key === "string") {
        this.updateProperty(
          property.name,
          this.convertRawPropertyValue(property, this.rawStation[property.key] as string)
        );
    ...
    rootHTTPLogger.debug("Update station cloud properties", {
    ...
    if (
      ...
      try {
        ...
        if (error instanceof InvalidPropertyError) {
          rootHTTPLogger.error(`Station update property - Invalid Property error`, {
          ...
          rootHTTPLogger.error(`Station update property - Property error`, {
            error: getError(error),
            stationSN: this.getSerial(),
            propertyName: name,
            propertyValue: value,
  ...
  public updateRawProperty(type: number, value: string, source: SourceType): boolean {
    ...
    if (
      ...
      ((this.rawProperties[type] !== undefined &&
        ...
        isPrioritySourceType(this.rawProperties[type].source, source)) ||
        this.rawProperties[type] === undefined)
    ) {
      ...
      if (this.ready) {
        ...
        try {
          ...
          const error = ensureError(err);
          rootHTTPLogger.error("Station update raw property - Number conversion error", {
            error: getError(error),
            stationSN: this.getSerial(),
            type: type,
            value: value,
        ...
        if (property.key === type) {
          try {
            ...
            if (error instanceof PropertyNotSupportedError) {
              rootHTTPLogger.debug("Station update raw property - Property not supported error", {
                error: getError(error),
                stationSN: this.getSerial(),
                type: type,
                value: value,
              ...
              rootHTTPLogger.error("Station update raw property - Property error", {
                error: getError(error),
                stationSN: this.getSerial(),
                type: type,
                value: value,
    ...
    try {
      switch (property.key) {
        ...
        case CommandType.CMD_GET_ALARM_MODE: {
          ...
          return Number.parseInt(
            value !== undefined
              ? value
              : guard_mode !== undefined && guard_mode !== GuardMode.SCHEDULE && guard_mode !== GuardMode.GEO
                ? (guard_mode as string)
                : GuardMode.UNKNOWN.toString()
          );
        }
          ...
          switch (property.name) {
            case PropertyName.StationNotificationSwitchModeSchedule:
              ...
              return value !== undefined
                ? isNotificationSwitchMode(Number.parseInt(value), NotificationSwitchMode.SCHEDULE)
                : false;
            case PropertyName.StationNotificationSwitchModeGeofence:
              ...
              return value !== undefined
                ? isNotificationSwitchMode(Number.parseInt(value), NotificationSwitchMode.GEOFENCE)
                : false;
            ...
            case PropertyName.StationNotificationSwitchModeKeypad:
              ...
              return value !== undefined
                ? isNotificationSwitchMode(Number.parseInt(value), NotificationSwitchMode.KEYPAD)
                : false;
          }
        }
        ...
        case CommandType.CMD_HUB_ALARM_TONE:
          try {
            ...
            const error = ensureError(err);
            rootHTTPLogger.error("Station convert raw property - CMD_HUB_ALARM_TONE Error", {
              error: getError(error),
              stationSN: this.getSerial(),
              property: property,
              value: value,
        ...
        case CommandType.CMD_SET_HUB_SPK_VOLUME:
          try {
            ...
            const error = ensureError(err);
            rootHTTPLogger.error("Station convert raw property - CMD_SET_HUB_SPK_VOLUME Error", {
              error: getError(error),
              stationSN: this.getSerial(),
              property: property,
              value: value,
        ...
        case CommandType.CMD_SET_PROMPT_VOLUME:
          try {
            ...
            const error = ensureError(err);
            rootHTTPLogger.error("Station convert raw property - CMD_SET_PROMPT_VOLUME Error", {
              error: getError(error),
              stationSN: this.getSerial(),
              property: property,
              value: value,
        ...
        case CommandType.CMD_SET_HUB_OSD:
          try {
            ...
            const error = ensureError(err);
            rootHTTPLogger.error("Station convert raw property - CMD_SET_HUB_OSD Error", {
              error: getError(error),
              stationSN: this.getSerial(),
              property: property,
              value: value,
      ...
      if (property.name === PropertyName.Model && Device.isLockWifiT8510P(this.getDeviceType(), this.getSerial())) {
        ...
        try {
          ...
          const error = ensureError(err);
          rootHTTPLogger.warn("Station convert raw property - PropertyMetadataNumeric Convert Error", {
            error: getError(error),
            stationSN: this.getSerial(),
            property: property,
            value: value,
        ...
        try {
          ...
          const error = ensureError(err);
          rootHTTPLogger.warn("Station convert raw property - PropertyMetadataBoolean Convert Error", {
            error: getError(error),
            stationSN: this.getSerial(),
            property: property,
            value: value,
      ...
      const error = ensureError(err);
      rootHTTPLogger.error("Station convert raw property - Error", {
        error: getError(error),
        stationSN: this.getSerial(),
        property: property,
        value: value,
  ...
  public getPropertyValue(name: string): PropertyValue {
    if (name === PropertyName.StationCurrentMode) {
      ...
      return this.properties[PropertyName.StationCurrentMode] !== undefined
        ? this.properties[PropertyName.StationCurrentMode]
        : guard_mode !== undefined && guard_mode !== GuardMode.SCHEDULE && guard_mode !== GuardMode.GEO
          ? guard_mode
          : GuardMode.UNKNOWN;
    }
  ...
  public getProperties(): PropertyValues {
    ...
    for (const property of Object.keys(this.properties)) {
      if (!property.startsWith("hidden-")) result[property] = this.properties[property];
    }
    return result;
  }
  ...
  public getPropertiesMetadata(hidden = false): IndexedProperty {
    ...
    if (!hidden) {
      for (const property of Object.keys(metadata)) {
        if (property.startsWith("hidden-")) delete metadata[property];
      }
    }
    return metadata;
  }
  ...
  public static getChannel(type: number): number {
    return Station.isStation(type) === true
      ? Device.isIndoorCamera(type)
        ? Station.CHANNEL_INDOOR
        : Station.CHANNEL
      : 0;
  }
  ...
  public isIntegratedDevice(): boolean {
    ...
    if (
      Device.isLock(this.getDeviceType()) ||
      Device.isSmartDrop(this.getDeviceType()) ||
      Device.isSmartSafe(this.getDeviceType())
    ) {
    ...
    return (
      Device.isWiredDoorbellDual(this.getDeviceType()) ||
      Device.isFloodLight(this.getDeviceType()) ||
      Device.isWiredDoorbell(this.getDeviceType()) ||
      Device.isIndoorCamera(this.getDeviceType()) ||
      Device.isSoloCameras(this.getDeviceType()) ||
      Device.isWallLightCam(this.getDeviceType()) ||
      Device.isStarlight4GLTE(this.getDeviceType()) ||
      Device.isSoloCameraPro(this.getDeviceType())
    );
  }
  ...
  public getCurrentMode(): PropertyValue {
    const guard_mode = this.getGuardMode();
    return this.getPropertyValue(PropertyName.StationCurrentMode) !== undefined
      ? this.getPropertyValue(PropertyName.StationCurrentMode)
      : guard_mode !== undefined && guard_mode !== GuardMode.SCHEDULE && guard_mode !== GuardMode.GEO
        ? guard_mode
        : GuardMode.UNKNOWN;
  }
  ...
  public processPushNotification(message: PushMessage): void {
    if (message.type !== undefined && message.event_type !== undefined) {
      if (message.event_type === CusPushEvent.MODE_SWITCH && message.station_sn === this.getSerial()) {
        rootHTTPLogger.info("Received push notification for changing guard mode", {
          guard_mode: message.station_guard_mode,
          current_mode: message.station_current_mode,
          stationSN: message.station_sn,
        });
        try {
          ...
          const error = ensureError(err);
          rootHTTPLogger.debug(`Station process push notification - MODE_SWITCH event error`, {
            error: getError(error),
            stationSN: this.getSerial(),
        ...
        message.event_type === CusPushEvent.ALARM &&
        message.station_sn === this.getSerial() &&
        !this.isStation()
      ) {
        rootHTTPLogger.info("Received push notification for alarm event", {
          stationSN: message.station_sn,
      ...
      message.msg_type === CusPushEvent.TFCARD &&
      message.station_sn === this.getSerial() &&
      message.tfcard_status !== undefined
    ) {
  ...
  public async connect(): Promise<void> {
    if (!this.p2pSession.isConnected() && !this.p2pSession.isConnecting()) {
      rootHTTPLogger.debug(`Connecting to station ${this.getSerial()}...`, {
        stationSN: this.getSerial(),
        p2pConnectionType: P2PConnectionType[this.p2pConnectionType],
      });
  ...
  public setGuardMode(mode: GuardMode): void {
    ...
    if (
      ...
      rootHTTPLogger.debug(`Station set guard mode - Using CMD_SET_ARMING`, {
        stationSN: this.getSerial(),
        mode: mode,
      });
  ...
  private getArmDelay(mode: AlarmMode): number {
    ...
    if (
      ...
      this.hasPropertyValue(propertyName) &&
      ...
      try {
        ...
        const error = ensureError(err);
        rootHTTPLogger.debug(`Station get arm delay - Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          mode: mode,
  ...
  private _getDeviceSerial(channel: number): string | undefined {
    if (this.rawStation.devices) {
      ...
      for (const device of this.rawStation.devices) {
        if (device.device_channel === channel) return device.device_sn;
      }
    } else {
      ...
      if (devices) {
        ...
        rootHTTPLogger.error(`Station get device serial - No devices found`, {
          station: this.getSerial(),
          channel: channel,
        });
      }
  ...
  private onCommandResponse(result: CommandResult): void {
    rootHTTPLogger.debug("Station got p2p command response", {
      station: this.getSerial(),
  ...
  private onSecondaryCommandResponse(result: CommandResult): void {
    rootHTTPLogger.debug("Station got p2p secondary command response", {
      station: this.getSerial(),
      commandType: result.command_type,
      channel: result.channel,
      returnCode: result.return_code,
    ...
    if (
      ...
      result.customData?.command?.name === "deviceVerifyPIN"
    ) {
  ...
  private onDisconnect(): void {
    ...
    this.pinVerified = false;
    if (!this.isEnergySavingDevice() && !this.terminating) this.scheduleReconnect();
  }
  ...
  public setStatusLed(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set status led - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (
      device.isCamera2Product() ||
      device.isCamera3Product() ||
      device.getDeviceType() === DeviceType.CAMERA ||
      device.getDeviceType() === DeviceType.CAMERA_E ||
      device.isCameraProfessional247() ||
      (device.isCameraC35() && this.isDeviceControlledByHomeBase())
    ) {
      this.p2pSession.sendCommandWithIntString(
        {
      ...
      (device.getDeviceType() === DeviceType.FLOODLIGHT && !device.isFloodLightT8420X()) ||
      device.isGarageCamera() ||
      (device.isIndoorPanAndTiltCameraS350() && this.isDeviceControlledByHomeBase()) ||
      (device.isSoloCameras() && this.isDeviceControlledByHomeBase())
    ) {
      this.p2pSession.sendCommandWithIntString(
        {
          commandType: CommandType.CMD_DEV_LED_SWITCH,
    ...
    } else if (device.isSoloCameras() && !this.isDeviceControlledByHomeBase()) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
          value: JSON.stringify({
  ...
  public setAutoNightVision(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set auto night vision - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNightVision(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set night vision - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMotionDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (
      device.isSoloCameraSolar() ||
      device.isOutdoorPanAndTiltCamera() ||
      device.isSoloCameraC210() ||
      ((device.isIndoorPanAndTiltCameraS350() || device.isCameraC35()) && !this.isDeviceControlledByHomeBase())
    ) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
      ...
      (device.isIndoorCamera() && !device.isIndoorPanAndTiltCameraS350()) ||
      (device.isFloodLight() && device.getDeviceType() !== DeviceType.FLOODLIGHT) ||
      device.isFloodLightT8420X() ||
      device.isWiredDoorbellT8200X() ||
      device.isStarlight4GLTE() ||
      device.isGarageCamera() ||
      device.isSoloCamera()
    ) {
      this.p2pSession.sendCommandWithStringPayload(
  ...
  public setSoundDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set sound detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSoundDetectionType(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set sound detection type - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSoundDetectionSensitivity(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set sound detection sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setPetDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set pet detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public panAndTilt(device: Device, direction: PanTiltDirection, command = 1): void {
    ...
    if (!(direction in PanTiltDirection)) {
      throw new InvalidCommandValueError("Invalid value for this command", {
        context: {
          device: device.getSerial(),
          station: this.getSerial(),
          commandName: commandData.name,
    ...
    rootHTTPLogger.debug(`Station pan and tilt - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
  ...
  public switchLight(device: Device, value: boolean): void {
    ...
    } else if (device.isWallLightCam() || (device.isCameraC35() && !this.isDeviceControlledByHomeBase())) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
          value: JSON.stringify({
  ...
  public setMotionDetectionSensitivity(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (
      (device.isFloodLight() && device.getDeviceType() !== DeviceType.FLOODLIGHT && !device.isFloodLightT8425()) ||
      (device.isIndoorCamera() && !device.isIndoorPanAndTiltCameraS350()) ||
      device.isFloodLightT8420X() ||
      device.isGarageCamera() ||
      (device.isIndoorPanAndTiltCameraS350() && !this.isDeviceControlledByHomeBase())
      ...
      (device.isBatteryDoorbell() && !device.isBatteryDoorbellDual()) ||
      device.isWiredDoorbellDual() ||
      (device.isBatteryDoorbell() && device.isBatteryDoorbellC30())
    ) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_SET_PAYLOAD,
  ...
  public setMotionDetectionType(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection type - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMotionDetectionTypeHB3(
    device: Device,
    type:
      | HB3DetectionTypes
      | T8170DetectionTypes
    ...
    const propertyData: PropertyData = {
      name:
        type === HB3DetectionTypes.HUMAN_RECOGNITION
          ? PropertyName.DeviceMotionDetectionTypeHumanRecognition
          : type === HB3DetectionTypes.HUMAN_DETECTION || type === T8170DetectionTypes.HUMAN_DETECTION
            ? PropertyName.DeviceMotionDetectionTypeHuman
            : type === HB3DetectionTypes.PET_DETECTION
              ? PropertyName.DeviceMotionDetectionTypePet
              : type === HB3DetectionTypes.VEHICLE_DETECTION
    ...
    rootHTTPLogger.debug(`Station set motion detection type HB3 - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      type: type,
      value: value,
    ...
    if (this.getDeviceType() === DeviceType.HB3) {
      try {
        if (!Object.values(HB3DetectionTypes).includes(type as HB3DetectionTypes)) {
          rootHTTPLogger.error(
            `The device ${device.getSerial()} accepts only this type of values:`,
            Object.values(HB3DetectionTypes).filter((value) => typeof value === "number")
          );
          return;
        }
        const aiDetectionType =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        let newAiDetectionType = getHB3DetectionMode(
          Number.parseInt(aiDetectionType),
          type as HB3DetectionTypes,
          value
        );
          ...
          },
          {
            ...
            onSuccess: () => {
              device.updateRawProperty(
                CommandType.CMD_SET_MOTION_DETECTION_TYPE_HB3,
                newAiDetectionType.toString(),
                "p2p"
              );
            },
          }
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setMotionDetectionTypeHB3 Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
      ...
      try {
        if (!Object.values(T8170DetectionTypes).includes(type as T8170DetectionTypes)) {
          rootHTTPLogger.error(
            `The device ${device.getSerial()} accepts only this type of values:`,
            Object.values(T8170DetectionTypes).filter((value) => typeof value === "number")
          );
          return;
        }
        const aiDetectionType =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        const newAiDetectionType = getT8170DetectionMode(
          Number.parseInt(aiDetectionType),
          type as T8170DetectionTypes,
          value
        );
        this.p2pSession.sendCommandWithStringPayload(
          {
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setMotionDetectionTypeHB3 T8170DetectionTypes Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
      ...
      try {
        if (!Object.values(EufyCamC35DetectionTypes).includes(type as EufyCamC35DetectionTypes)) {
          rootHTTPLogger.error(
            `The device ${device.getSerial()} accepts only this type of values:`,
            Object.values(EufyCamC35DetectionTypes).filter((value) => typeof value === "number")
          );
          return;
        }
        const aiDetectionType =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        const newAiDetectionType = getT8110DetectionMode(
          Number.parseInt(aiDetectionType),
          type as EufyCamC35DetectionTypes,
          value
        );
        this.p2pSession.sendCommandWithStringPayload(
          {
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setMotionDetectionTypeHB3 EufyCamC35DetectionTypes Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
      ...
      try {
        if (!Object.values(SoloCameraDetectionTypes).includes(type as SoloCameraDetectionTypes)) {
          rootHTTPLogger.error(
            `The device ${device.getSerial()} accepts only this type of values:`,
            Object.values(SoloCameraDetectionTypes).filter((value) => typeof value === "number")
          );
          return;
        }
        ...
        if (!value) {
          newAiDetectionType =
            type === SoloCameraDetectionTypes.ALL_OTHER_MOTION
              ? SoloCameraDetectionTypes.HUMAN_DETECTION
              : SoloCameraDetectionTypes.ALL_OTHER_MOTION;
        }
        this.p2pSession.sendCommandWithStringPayload(
          {
            commandType: CommandType.CMD_SET_PAYLOAD,
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setMotionDetectionTypeHB3 SoloCameraDetectionTypes Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
      ...
      try {
        if (!Object.values(IndoorS350DetectionTypes).includes(type as IndoorS350DetectionTypes)) {
          rootHTTPLogger.error(
            `The device ${device.getSerial()} accepts only this type of values:`,
            Object.values(IndoorS350DetectionTypes).filter((value) => typeof value === "number")
          );
          return;
        }
        const aiDetectionType =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        const newAiDetectionType = getIndoorS350DetectionMode(
          Number.parseInt(aiDetectionType),
          type as IndoorS350DetectionTypes,
          value
        );
        this.p2pSession.sendCommandWithStringPayload(
          {
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setMotionDetectionTypeHB3 IndoorS350DetectionTypes Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
  ...
  public setMotionZone(device: Device, value: MotionZone): void {
    ...
    rootHTTPLogger.debug(`Station set motion zone - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMotionTracking(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion tracking - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setPanAndTiltRotationSpeed(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set pan and tilt rotation speed - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMicMute(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set mic mute - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAudioRecording(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set audio recording - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public enableSpeaker(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station enable speaker - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSpeakerVolume(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set speaker volume - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setRingtoneVolume(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set ringtone volume - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public enableIndoorChime(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station enable indoor chime - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public enableHomebaseChime(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station enable homebase chime - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setHomebaseChimeRingtoneVolume(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set homebase chime ringtone volume - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setHomebaseChimeRingtoneType(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set homebase chime ringtone type - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationType(device: Device, value: NotificationType | WalllightNotificationType): void {
    ...
    rootHTTPLogger.debug(`Station set notification type - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (
      ...
      if (!Object.values(NotificationType).includes(value as NotificationType)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(NotificationType).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithStringPayload(
    ...
    } else if (device.isIndoorPanAndTiltCameraS350() && !this.isDeviceControlledByHomeBase()) {
      if (!Object.values(NotificationType).includes(value as NotificationType)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(NotificationType).filter((value) => typeof value === "number")
      ...
      if (!Object.values(WalllightNotificationType).includes(value as WalllightNotificationType)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(WalllightNotificationType).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithStringPayload(
      ...
      this.p2pSession.sendCommandWithStringPayload(
        {
          ...
          value: JSON.stringify({
            ...
            mValue3: 0,
            payload: {
              notification_motion_onoff:
                device.getPropertyValue(PropertyName.DeviceNotificationMotion) === true ? 1 : 0,
  ...
  public setNotificationPerson(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification person - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationPet(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification pet - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationAllOtherMotion(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification all other motion - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationAllSound(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification all sound - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationCrying(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification crying - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationRing(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification ring - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
      ...
      this.p2pSession.sendCommandWithStringPayload(
        {
          ...
          value: JSON.stringify({
            ...
            data: {
              type:
                value === true
                  ? (device.getPropertyValue(PropertyName.DeviceNotificationMotion) as boolean) === true
                    ? 3
                    : 1
                  : (device.getPropertyValue(PropertyName.DeviceNotificationMotion) as boolean) === true
                    ? 2
  ...
  public setNotificationMotion(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification motion - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
      ...
      this.p2pSession.sendCommandWithStringPayload(
        {
          ...
          value: JSON.stringify({
            ...
            data: {
              type:
                value === true
                  ? (device.getPropertyValue(PropertyName.DeviceNotificationRing) as boolean) === true
                    ? 3
                    : 2
                  : (device.getPropertyValue(PropertyName.DeviceNotificationRing) as boolean) === true
                    ? 1
  ...
  public setPowerSource(device: Device, value: PowerSource): void {
    ...
    rootHTTPLogger.debug(`Station set power source - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    ...
    } else if (device.isOutdoorPanAndTiltCamera() || (device.isSoloCameras() && !this.isDeviceControlledByHomeBase())) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
          value: JSON.stringify({
  ...
  public setPowerWorkingMode(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set power working mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setRecordingClipLength(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set recording clip length - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setRecordingRetriggerInterval(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set recording retrigger interval - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setRecordingEndClipMotionStops(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set recording end clip motion stops - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setVideoStreamingQuality(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set video streaming quality - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    ...
    if (device.isBatteryDoorbellDualE340()) {
      ...
      device.isIndoorCamera() ||
      (device.isSoloCameras() && !this.isDeviceControlledByHomeBase()) ||
      device.isFloodLight() ||
      device.isWiredDoorbell() ||
      device.isStarlight4GLTE() ||
      device.isGarageCamera()
    ) {
      this.p2pSession.sendCommandWithStringPayload(
  ...
  public setVideoRecordingQuality(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set video recording quality - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    ...
    } else if ((device.isSoloCameras() && this.isDeviceControlledByHomeBase()) || device.isLockWifiVideo()) {
      this.p2pSession.sendCommandWithStringPayload(
        {
          commandType: CommandType.CMD_SET_PAYLOAD,
          value: JSON.stringify({
  ...
  public setWDR(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set wdr - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setFloodlightLightSettingsEnable(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set light settings enable - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setFloodlightLightSettingsBrightnessManual(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings brightness manual - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setFloodlightLightSettingsBrightnessMotion(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings brightness motion - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isFloodLightT8425()) {
      const rawProperty = device.getRawProperty(
        CommandType.CMD_SET_LIGHT_CTRL_BRIGHT_PIR_T8425
      ) as unknown as FloodlightLightSettingsMotionT8425Property;
      if (rawProperty !== undefined) {
        ...
        rootHTTPLogger.error(
          `Station set light settings brightness motion - Needed raw property "${CommandType.CMD_SET_LIGHT_CTRL_BRIGHT_PIR_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
  ...
  public setFloodlightLightSettingsBrightnessSchedule(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings brightness schedule - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setFloodlightLightSettingsMotionTriggered(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion triggered - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isFloodLightT8425() || device.isCameraC35()) {
      ...
      if (rawProperty !== undefined) {
        ...
        rootHTTPLogger.error(
          `Station set light settings motion triggered - Needed raw property "${CommandType.CMD_SET_LIGHT_CTRL_BRIGHT_PIR_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else if (device.isFloodLight()) {
  ...
  public setFloodlightLightSettingsMotionTriggeredDistance(
    device: Device,
    value: FloodlightMotionTriggeredDistance
  ): void {
    const propertyData: PropertyData = {
    ...
    switch (value as number) {
      case 1:
        newValue = FloodlightMotionTriggeredDistance.MIN;
        break;
      case 2:
        newValue = FloodlightMotionTriggeredDistance.LOW;
        break;
      case 3:
        newValue = FloodlightMotionTriggeredDistance.MEDIUM;
        break;
      case 4:
        newValue = FloodlightMotionTriggeredDistance.HIGH;
        break;
      case 5:
        newValue = FloodlightMotionTriggeredDistance.MAX;
        break;
      default:
        throw new InvalidPropertyValueError("Invalid value for this property", {
          context: {
            device: device.getSerial(),
            station: this.getSerial(),
            propertyName: propertyData.name,
    ...
    rootHTTPLogger.debug(`Station set light settings motion triggered distance - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: newValue,
    });
  ...
  public setFloodlightLightSettingsMotionTriggeredTimer(device: Device, seconds: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion triggered timer - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: seconds,
    });
    if (device.isFloodLightT8425()) {
      ...
      if (rawProperty !== undefined) {
        ...
        rootHTTPLogger.error(
          `Station set light settings motion triggered timer - Needed raw property "${CommandType.CMD_SET_LIGHT_CTRL_BRIGHT_PIR_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else if (device.isFloodLight()) {
  ...
  public triggerStationAlarmSound(seconds: number): void {
    ...
    rootHTTPLogger.debug(`Station trigger station alarm sound - sending command`, {
      stationSN: this.getSerial(),
      value: seconds,
    });
    if (
  ...
  public triggerDeviceAlarmSound(device: Device, seconds: number): void {
    ...
    rootHTTPLogger.debug(`Station trigger device alarm sound - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: seconds,
    });
  ...
  public setStationAlarmRingtoneVolume(value: number): void {
    ...
    rootHTTPLogger.debug(`Station set station alarm ringtone volume - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
  ...
  public setStationAlarmTone(value: AlarmTone): void {
    ...
    rootHTTPLogger.debug(`Station set station alarm tone - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setStationPromptVolume(value: number): void {
    ...
    rootHTTPLogger.debug(`Station set station prompt volume - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setStationNotificationSwitchMode(mode: NotificationSwitchMode, value: boolean): void {
    const propertyData: PropertyData = {
      name:
        mode === NotificationSwitchMode.APP
          ? PropertyName.StationNotificationSwitchModeApp
          : mode === NotificationSwitchMode.GEOFENCE
            ? PropertyName.StationNotificationSwitchModeGeofence
            : mode === NotificationSwitchMode.KEYPAD
              ? PropertyName.StationNotificationSwitchModeKeypad
              : mode === NotificationSwitchMode.SCHEDULE
                ? PropertyName.StationNotificationSwitchModeSchedule
                : ("" as PropertyName),
      value: value,
    };
    if (
      (!this.hasProperty(PropertyName.StationNotificationSwitchModeApp) && mode === NotificationSwitchMode.APP) ||
      (!this.hasProperty(PropertyName.StationNotificationSwitchModeGeofence) &&
        mode === NotificationSwitchMode.GEOFENCE) ||
      (!this.hasProperty(PropertyName.StationNotificationSwitchModeKeypad) && mode === NotificationSwitchMode.KEYPAD) ||
      (!this.hasProperty(PropertyName.StationNotificationSwitchModeSchedule) &&
        mode === NotificationSwitchMode.SCHEDULE)
    ) {
      throw new NotSupportedError("This functionality is not implemented or supported", {
        context: { propertyName: propertyData.name, propertyValue: propertyData.value, station: this.getSerial() },
    ...
    rootHTTPLogger.debug(`Station set station notification switch mode - sending command`, {
      stationSN: this.getSerial(),
      mode: mode,
      value: value,
    });
    if (this.isStation() && isGreaterEqualMinVersion("2.1.1.6", this.getSoftwareVersion())) {
      ...
      this.p2pSession.sendCommandWithStringPayload(
        {
          ...
          value: JSON.stringify({
            ...
            payload: {
              ...
              notify_alarm_delay:
                this.getPropertyValue(PropertyName.StationNotificationStartAlarmDelay) !== undefined
                  ? this.getPropertyValue(PropertyName.StationNotificationStartAlarmDelay) === true
                    ? 1
                    : 0
                  : 0,
              notify_mode: 0,
      ...
      this.getDeviceType() === DeviceType.OUTDOOR_PT_CAMERA ||
      Device.isSoloCameraC210(this.getDeviceType()) ||
      (Device.isCameraC35(this.getDeviceType()) && !this.isDeviceControlledByHomeBase())
    ) {
      let oldvalue = 0;
      const rawproperty = this.getRawProperty(CommandType.CMD_HUB_NOTIFY_MODE);
      if (rawproperty !== undefined) {
  ...
  public setStationNotificationStartAlarmDelay(value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set station notification start alarm delay - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
  ...
  public setStationTimeFormat(value: TimeFormat): void {
    ...
    rootHTTPLogger.debug(`Station set station time format - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    if (Device.isWallLightCam(this.getDeviceType())) {
  ...
  public setRTSPStream(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set rtsp stream - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAntiTheftDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set anti theft detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setWatermark(
    device: Device,
    value: WatermarkSetting1 | WatermarkSetting2 | WatermarkSetting3 | WatermarkSetting4 | WatermarkSetting5
  ): void {
    const propertyData: PropertyData = {
    ...
    rootHTTPLogger.debug(`Station set watermark - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (
      ...
      if (!Object.values(WatermarkSetting3).includes(value as WatermarkSetting3)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(WatermarkSetting3).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithIntString(
      ...
      if (!Object.values(WatermarkSetting1).includes(value as WatermarkSetting1)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(WatermarkSetting1).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithIntString(
      ...
      if (!Object.values(WatermarkSetting4).includes(value as WatermarkSetting4)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(WatermarkSetting4).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithIntString(
      ...
      if (!Object.values(WatermarkSetting2).includes(value as WatermarkSetting2)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values: `,
          Object.values(WatermarkSetting2).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithIntString(
      ...
      if (!Object.values(WatermarkSetting5).includes(value as WatermarkSetting5)) {
        rootHTTPLogger.error(
          `The device ${device.getSerial()} accepts only this type of values:`,
          Object.values(WatermarkSetting5).filter((value) => typeof value === "number")
        );
        return;
      }
      this.p2pSession.sendCommandWithIntString(
  ...
  public enableDevice(device: Device, value: boolean): void {
    ...
    if (
      (device.isIndoorCamera() && !device.isIndoorCamMini() && !device.isIndoorPanAndTiltCameraS350()) ||
      (device.isWiredDoorbell() && !device.isWiredDoorbellT8200X()) ||
      device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8422 ||
      device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8424 ||
      device.isFloodLightT8420X()
    )
      param_value = value === true ? 1 : 0;

    rootHTTPLogger.debug(`Station enable device - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isIndoorCamMini()) {
      ...
      device.isIndoorCamera() &&
      isGreaterEqualMinVersion("2.3.1.0", device.getSoftwareVersion()) &&
      Station.isDeviceControlledByHomeBaseBySn(device.getStationSerial())
    ) {
  ...
  public async startDownload(device: Device, path: string, cipher_id?: number): Promise<void> {
    ...
    if (this.getDeviceType() === DeviceType.HB3) {
      ...
      const cipher = await this.api.getCipher(
  ...
  public startLivestream(device: Device, videoCodec: VideoCodec = VideoCodec.H264): void {
    ...
    if (device.isSmartDrop()) {
      rootHTTPLogger.debug(`Station start livestream - sending command (smart drop)`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        videoCodec: videoCodec,
        main_sw_version: this.getSoftwareVersion(),
      ...
      rootHTTPLogger.debug(`Station start livestream - sending command using CMD_DOORBELL_SET_PAYLOAD (2)`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        videoCodec: videoCodec,
        main_sw_version: this.getSoftwareVersion(),
      ...
      rootHTTPLogger.debug(`Station start livestream - sending command using CMD_DOORBELL_SET_PAYLOAD (4)`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        videoCodec: videoCodec,
        main_sw_version: this.getSoftwareVersion(),
      ...
      (device.isFloodLight() && device.getDeviceType() !== DeviceType.FLOODLIGHT) ||
      device.isIndoorCamera() ||
      (device.getSerial().startsWith("T8420") && isGreaterEqualMinVersion("2.0.4.8", this.getSoftwareVersion()))
    ) {
      rootHTTPLogger.debug(`Station start livestream - sending command using CMD_DOORBELL_SET_PAYLOAD (3)`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        videoCodec: videoCodec,
        main_sw_version: this.getSoftwareVersion(),
  ...
  public stopLivestream(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station stop livestream - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    this.p2pSession.sendCommandWithInt(
  ...
  public quickResponse(device: Device, voice_id: number): void {
    ...
    rootHTTPLogger.debug(`Station quick response - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      voiceID: voice_id,
    });
    if (device.isBatteryDoorbell() || device.isLockWifiVideo()) {
      rootHTTPLogger.debug(`Station quick response - sending command using CMD_BAT_DOORBELL_QUICK_RESPONSE`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        voiceID: voice_id,
      });
      ...
      rootHTTPLogger.debug(`Station quick response - sending command using CMD_DOORBELL_SET_PAYLOAD`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        voiceID: voice_id,
      });
  ...
  public setChirpVolume(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set chirp volume - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setChirpTone(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set chirp tone - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setHDR(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set hdr - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDistortionCorrection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set distortion correction - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setRingRecord(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set ring record - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public lockDevice(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station lock device - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isLockBleNoFinger() || device.isLockBle()) {
      ...
      const lockCmd = Lock.encodeESLCmdOnOff(
        Number.parseInt(this.rawStation.member.short_user_id),
      ...
      rootHTTPLogger.debug("Station lock device - Locking/unlocking device...", {
        ...
        payload: payload,
        encPayload: encPayload.toString("hex"),
      });
    ...
    } else if (device.usesSecurityMqtt()) {
      rootHTTPLogger.debug("Station lock device - Using security MQTT...", {
        station: this.getSerial(),
        device: device.getSerial(),
        value: value,
      });
  ...
  public setStationSwitchModeWithAccessCode(value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set station switch mode with access code - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    if (this.isStation()) {
  ...
  public setStationAutoEndAlarm(value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station station auto end alarm - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    if (this.isStation()) {
  ...
  public setStationTurnOffAlarmWithButton(value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set station turn off alarm with button - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    if (this.isStation()) {
  ...
  public startRTSPStream(device: Device): void {
    ...
    if (rtspStreamProperty !== undefined && rtspStreamProperty !== true) {
      throw new RTSPPropertyNotEnabledError(
        "RTSP setting for this device must be enabled first, to enable this functionality!",
        {
          context: {
            device: device.getSerial(),
            station: this.getSerial(),
            propertyName: PropertyName.DeviceRTSPStream,
            propertyValue: rtspStreamProperty,
          },
        }
      );
    }
    ...
    rootHTTPLogger.debug(`Station start rtsp stream - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    this.p2pSession.sendCommandWithIntString(
  ...
  public stopRTSPStream(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station stop rtsp stream - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    this.p2pSession.sendCommandWithIntString(
  ...
  public setMotionDetectionRange(device: Device, type: MotionDetectionRangeType): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection range - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: type,
    });
    if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423) {
      ...
      ) as unknown as FloodlightDetectionRangeT8425Property;
      if (rawProperty !== undefined) {
        const payload: FloodlightDetectionRangeT8425Property = {
          ...rawProperty,
        };
        const currentMode =
          type === 0 ? payload.mode0 : type === 1 ? payload.mode1 : type === 2 ? payload.mode2 : undefined;
        if (currentMode !== undefined && Array.isArray(currentMode) && currentMode.length === 4) {
          ...
          rootHTTPLogger.error(
            `Station set motion detection range - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} has unexpected value`,
            { stationSN: this.getSerial(), rawProperty: rawProperty }
          );
        }
      } else {
        rootHTTPLogger.error(
          `Station set motion detection range - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else {
  ...
  public setMotionDetectionRangeStandardSensitivity(device: Device, sensitivity: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection range standard sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: sensitivity,
    });
    if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423) {
      ...
      if (rawProperty !== undefined) {
        if (rawProperty.mode0 !== undefined && Array.isArray(rawProperty.mode0) && rawProperty.mode0.length === 4) {
          ...
          rootHTTPLogger.error(
            `Station set motion detection range standard sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} has unexpected value`,
            { stationSN: this.getSerial(), rawProperty: rawProperty }
          );
        }
      } else {
        rootHTTPLogger.error(
          `Station set motion detection range standard sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else {
  ...
  public setMotionDetectionRangeAdvancedLeftSensitivity(device: Device, sensitivity: number): void {
    ...
    rootHTTPLogger.debug(`Station motion detection range advanced left sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: sensitivity,
    });
    if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423) {
      ...
      if (rawProperty !== undefined) {
        if (rawProperty.mode1 !== undefined && Array.isArray(rawProperty.mode1) && rawProperty.mode1.length === 4) {
          ...
          rootHTTPLogger.error(
            `Station set motion detection range advanced left sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} has unexpected value`,
            { stationSN: this.getSerial(), rawProperty: rawProperty }
          );
        }
      } else {
        rootHTTPLogger.error(
          `Station set motion detection range advanced left sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else {
  ...
  public setMotionDetectionRangeAdvancedMiddleSensitivity(device: Device, sensitivity: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection range advanced middle sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: sensitivity,
    });
  ...
  public setMotionDetectionRangeAdvancedRightSensitivity(device: Device, sensitivity: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection range advanced right sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: sensitivity,
    });
    if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423) {
      ...
      if (rawProperty !== undefined) {
        if (rawProperty.mode1 !== undefined && Array.isArray(rawProperty.mode1) && rawProperty.mode1.length === 4) {
          ...
          rootHTTPLogger.error(
            `Station set motion detection range advanced right sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} has unexpected value`,
            { stationSN: this.getSerial(), rawProperty: rawProperty }
          );
        }
      } else {
        rootHTTPLogger.error(
          `Station set motion detection range advanced right sensitivity - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else {
  ...
  public setMotionDetectionTestMode(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection test mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
    if (
      device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423 ||
      device.getDeviceType() === DeviceType.FLOODLIGHT
    ) {
      this.p2pSession.sendCommandWithIntString(
        {
      ...
      if (rawProperty !== undefined && rawProperty.cur_mode !== undefined) {
        ...
        if (currentMode !== undefined && Array.isArray(currentMode) && currentMode.length === 4) {
          ...
          rootHTTPLogger.error(
            `Station set motion detection test mode - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} has unexpected value`,
            { stationSN: this.getSerial(), rawProperty: rawProperty }
          );
        }
      } else {
        rootHTTPLogger.error(
          `Station set motion detection test mode - Needed raw property "${CommandType.CMD_FLOODLIGHT_SET_DETECTION_RANGE_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
    } else {
  ...
  public setMotionTrackingSensitivity(device: Device, sensitivity: number): void {
    ...
    rootHTTPLogger.debug(`Station set motion tracking sensitivity - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: sensitivity,
    });
  ...
  public setMotionAutoCruise(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion auto cruise - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
  ...
  public setMotionOutOfViewDetection(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion out of view detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
  ...
  public setLightSettingsColorTemperatureManual(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings color temperature manual - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsColorTemperatureMotion(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings color temperature motion - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsColorTemperatureSchedule(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings color temperature schedule - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsMotionActivationMode(device: Device, value: MotionActivationMode): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion activation mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isFloodLightT8425()) {
      ...
      if (rawProperty !== undefined) {
        ...
        rootHTTPLogger.error(
          `Station set light settings motion activation mode - Needed raw property "${CommandType.CMD_SET_LIGHT_CTRL_BRIGHT_PIR_T8425}" on device ${device.getSerial()} is undefined`,
          { stationSN: this.getSerial() }
        );
      }
  ...
  public setVideoNightvisionImageAdjustment(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set video night vision image adjustment - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
  ...
  public setVideoColorNightvision(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set video color night vision - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
  ...
  public setAutoCalibration(device: Device, enabled: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set auto calibration - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: enabled,
    });
  ...
  public hasDevice(deviceSN: string): boolean {
    if (this.rawStation.devices)
      for (const device of this.rawStation.devices) {
        if (device.device_sn === deviceSN) return true;
      }
    return false;
  }
  ...
  public hasDeviceWithType(deviceType: DeviceType): boolean {
    if (this.rawStation.devices)
      for (const device of this.rawStation.devices) {
        if (device.device_type === deviceType) return true;
      }
    return false;
  }
  ...
  public calibrateLock(device: Device): void {
    ...
    if (device.getStationSerial() !== this.getSerial()) {
      throw new WrongStationError("Device is not managed by this station", {
        context: { device: device.getSerial(), station: this.getSerial(), commandName: commandData.name },
      });
    }
    ...
    rootHTTPLogger.debug(`Station calibrate lock - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      rootHTTPLogger.debug("Station calibrate lock - Calibrate lock...", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
  ...
  private getAdvancedLockSettingsPayload(command: CommandType, device: Device): AdvancedLockSetParamsType {
    ...
    return {
      autoLockTime: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceAutoLockTimer,
        device.getPropertyValue(PropertyName.DeviceAutoLockTimer)
      ) as number,
      isAutoLock: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceAutoLock,
        device.getPropertyValue(PropertyName.DeviceAutoLock)
      ) as number,
      isLockNotification: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceNotificationLocked,
        device.getPropertyValue(PropertyName.DeviceNotificationLocked)
      ) as number,
      isNotification: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceNotification,
        device.getPropertyValue(PropertyName.DeviceNotification)
      ) as number,
      isOneTouchLock: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceOneTouchLocking,
        device.getPropertyValue(PropertyName.DeviceOneTouchLocking)
      ) as number,
      isSchedule: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceAutoLockSchedule,
        device.getPropertyValue(PropertyName.DeviceAutoLockSchedule)
      ) as number,
      isScramblePasscode: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceScramblePasscode,
        device.getPropertyValue(PropertyName.DeviceScramblePasscode)
      ) as number,
      isUnLockNotification: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceNotificationUnlocked,
        device.getPropertyValue(PropertyName.DeviceNotificationUnlocked)
      ) as number,
      isWrongTryProtect: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceWrongTryProtection,
        device.getPropertyValue(PropertyName.DeviceWrongTryProtection)
      ) as number,
      lockDownTime: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceWrongTryLockdownTime,
        device.getPropertyValue(PropertyName.DeviceWrongTryLockdownTime)
      ) as number,
      lockSound: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceSound,
        device.getPropertyValue(PropertyName.DeviceSound)
      ) as number,
      paramType: command,
      scheduleEnd: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceAutoLockScheduleEndTime,
        device.getPropertyValue(PropertyName.DeviceAutoLockScheduleEndTime)
      ) as string,
      scheduleStart: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceAutoLockScheduleStartTime,
        device.getPropertyValue(PropertyName.DeviceAutoLockScheduleStartTime)
      ) as string,
      wrongTryTime: this.convertAdvancedLockSettingValue(
        PropertyName.DeviceWrongTryAttempts,
        device.getPropertyValue(PropertyName.DeviceWrongTryAttempts)
      ) as number,
      seq_num: this.p2pSession.incLockSequenceNumber(),
    };
  }
  ...
  private getAdvancedLockSettingsPayloadT8530(command: CommandType, device: Device): AdvancedLockSetParamsTypeT8520 {
    ...
    return {
      autoLockTime: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceAutoLockTimer,
        device.getPropertyValue(PropertyName.DeviceAutoLockTimer)
      ) as number,
      isAutoLock: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceAutoLock,
        device.getPropertyValue(PropertyName.DeviceAutoLock)
      ) as number,
      isLockNotification: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceNotificationLocked,
        device.getPropertyValue(PropertyName.DeviceNotificationLocked)
      ) as number,
      isNotification: 1, // unused constant
      isOneTouchLock: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceOneTouchLocking,
        device.getPropertyValue(PropertyName.DeviceOneTouchLocking)
      ) as number,
      isSchedule: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceAutoLockSchedule,
        device.getPropertyValue(PropertyName.DeviceAutoLockSchedule)
      ) as number,
      isScramblePasscode: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceScramblePasscode,
        device.getPropertyValue(PropertyName.DeviceScramblePasscode)
      ) as number,
      isUnLockNotification: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceNotificationUnlocked,
        device.getPropertyValue(PropertyName.DeviceNotificationUnlocked)
      ) as number,
      isWrongTryProtect: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceWrongTryProtection,
        device.getPropertyValue(PropertyName.DeviceWrongTryProtection)
      ) as number,
      lockDownTime: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceWrongTryLockdownTime,
        device.getPropertyValue(PropertyName.DeviceWrongTryLockdownTime)
      ) as number,
      lockOpenDirection: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceNightvisionOptimizationSide,
        device.getPropertyValue(PropertyName.DeviceNightvisionOptimizationSide)
      ) as number,
      lockSound: 1, // unused constant
      lockVolume: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceBeepVolume,
        device.getPropertyValue(PropertyName.DeviceBeepVolume)
      ) as number,
      nightVisionEnhance: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceNightvisionOptimization,
        device.getPropertyValue(PropertyName.DeviceNightvisionOptimization)
      ) as number,
      ...
      paramType: command,
      scheduleEnd: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceAutoLockScheduleEndTime,
        device.getPropertyValue(PropertyName.DeviceAutoLockScheduleEndTime)
      ) as string,
      scheduleStart: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceAutoLockScheduleStartTime,
        device.getPropertyValue(PropertyName.DeviceAutoLockScheduleStartTime)
      ) as string,
      ...
      wrongTryTime: this.convertAdvancedLockSettingValueT8530(
        PropertyName.DeviceWrongTryAttempts,
        device.getPropertyValue(PropertyName.DeviceWrongTryAttempts)
      ) as number,
    };
  ...
  private getAdvancedLockSettingName(property: PropertyName): string {
    switch (property) {
      ...
      case PropertyName.DeviceNotificationLocked:
        return "isLockNotification";
      case PropertyName.DeviceOneTouchLocking:
        return "isOneTouchLock";
      case PropertyName.DeviceAutoLockSchedule:
        return "isSchedule";
      case PropertyName.DeviceScramblePasscode:
        return "isScramblePasscode";
      case PropertyName.DeviceNotificationUnlocked:
        return "isUnLockNotification";
      case PropertyName.DeviceWrongTryProtection:
  ...
  public setAdvancedLockParams(device: Device, property: PropertyName, value: PropertyValue): void {
    ...
    rootHTTPLogger.debug(`Station set advanced lock params - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      property: property,
      value: value,
    ...
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      if (p2pParamName !== "") {
        ...
        rootHTTPLogger.debug("Station set advanced lock params - Set lock param...", {
          station: this.getSerial(),
          device: device.getSerial(),
          admin_user_id: this.rawStation.member.admin_user_id,
          property: property,
        ...
        rootHTTPLogger.warn(
          `Internal lock property for property ${property} not identified for ${device.getSerial()}`,
          { p2pParamName: p2pParamName }
        );
        throw new InvalidPropertyError("Internal lock property for property not identified for this device", {
          context: {
            device: device.getSerial(),
            station: this.getSerial(),
            propertyName: propertyData.name,
      ...
      const payload = this.getAdvancedLockSettingsPayloadT8530(
        device.getPropertyMetadata(property).key as number,
        device
      );
  ...
  public setLoiteringDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set loitering detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLoiteringDetectionRange(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set loitering detection range - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLoiteringDetectionLength(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set loitering detection length - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  private _setMotionDetectionSensitivity(
    ...
    rootHTTPLogger.debug(`Station set motion detection sensitivty - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyData: propertyData,
      mode: mode,
  ...
  public setMotionDetectionSensitivityStandard(device: Device, value: number): void {
    ...
    this._setMotionDetectionSensitivity(
      device,
      propertyData,
      MotionDetectionMode.STANDARD,
      getBlocklist(Array(8).fill(value))
    );
  }
  ...
  private _setLoiteringCustomResponse(
    ...
    rootHTTPLogger.debug(`Station set loitering custom response - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyData: propertyData,
      voiceID: voiceID,
      autoVoiceResponse: autoVoiceResponse,
      homebaseAlert: homebaseAlert,
  ...
  public setDeliveryGuard(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDeliveryGuardPackageGuarding(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard package guarding - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDeliveryGuardPackageGuardingVoiceResponseVoice(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard package guarding voice response voice - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  private setDeliveryGuardPackageGuardingActivatedTime(
    ...
    rootHTTPLogger.debug(`Station set delivery guard guarding activated time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyData: propertyData,
      startTime: start,
  ...
  public setDeliveryGuardUncollectedPackageAlert(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard uncollected package alert - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDeliveryGuardUncollectedPackageAlertTimeToCheck(device: Device, value: string): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard uncollected package alert time to check - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDeliveryGuardPackageLiveCheckAssistance(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set delivery guard package live check assistance - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDualCamWatchViewMode(device: Device, value: ViewModeType): void {
    ...
    rootHTTPLogger.debug(`Station set dual cam watch view mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  private _setRingAutoResponse(
    ...
    rootHTTPLogger.debug(`Station set ring auto response - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyData: propertyData,
      enabled: enabled,
  ...
  public setNotificationRadarDetector(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification radar detector - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public calibrate(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station calibrate - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
  ...
  public setContinuousRecording(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set continuous recording - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setContinuousRecordingType(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set continuous recording type - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public enableDefaultAngle(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station enable default angle - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDefaultAngleIdleTime(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set default angle idle time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setDefaultAngle(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station set default angle - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setPrivacyAngle(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station set privacy angle - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setNotificationIntervalTime(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set notification interval time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSoundDetectionRoundLook(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set sound detection round look - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public startTalkback(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station start talkback - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    if (
      (device.isIndoorCamera() && !device.isIndoorPanAndTiltCameraS350()) ||
      (device.isSoloCamera() && !this.isDeviceControlledByHomeBase()) ||
      (device.isFloodLight() && !device.isFloodLightT8425()) ||
      device.isWiredDoorbell() ||
      device.isStarlight4GLTE() ||
      device.isWallLightCam() ||
      ...
      (device.isBatteryDoorbell() && isGreaterEqualMinVersion("2.0.6.8", this.getSoftwareVersion())) ||
      device.isIndoorPanAndTiltCameraS350() ||
      device.isFloodLightT8425() ||
      (device.isSoloCamera() && this.isDeviceControlledByHomeBase()) ||
      device.isLockWifiVideo() ||
  ...
  public stopTalkback(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station stop talkback - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    if (
  ...
  public setScramblePasscode(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set scramble passcode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setWrongTryProtection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set wrong try protection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setWrongTryAttempts(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set wrong try attempts - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setWrongTryLockdownTime(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set wrong try lockdown time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSmartSafeParams(device: Device, property: PropertyName, value: PropertyValue): void {
    ...
    rootHTTPLogger.debug(`Station set smart safe params - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      property: property,
      value: value,
    ...
    if (device.isSmartSafe()) {
      ...
      switch (property) {
        ...
        case PropertyName.DeviceDualUnlock:
          payload = SmartSafe.encodeCmdDualUnlock(this.rawStation.member.admin_user_id, value as boolean);
          command = SmartSafeCommandCode.SET_DUAL_UNLOCK;
          break;
        case PropertyName.DevicePowerSave:
          payload = SmartSafe.encodeCmdPowerSave(this.rawStation.member.admin_user_id, value as boolean);
        ...
        case PropertyName.DeviceTamperAlarm:
          payload = SmartSafe.encodeCmdTamperAlarm(this.rawStation.member.admin_user_id, value as number);
        ...
        case PropertyName.DeviceRemoteUnlockMasterPIN: {
          ...
          let newValue = 2;
          const remoteUnlock =
            property === PropertyName.DeviceRemoteUnlock
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceRemoteUnlock) as boolean);
          const remoteUnlockMasterPIN =
            property === PropertyName.DeviceRemoteUnlockMasterPIN
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceRemoteUnlockMasterPIN) as boolean);
          if (remoteUnlock && remoteUnlockMasterPIN) {
            newValue = 0;
          } else if (remoteUnlock) {
            newValue = 1;
          }
          payload = SmartSafe.encodeCmdRemoteUnlock(this.rawStation.member.admin_user_id, newValue);
          command = SmartSafeCommandCode.SET_UNLOCK_MODE;
          break;
        }
        case PropertyName.DevicePromptVolume:
          payload = SmartSafe.encodeCmdPromptVolume(this.rawStation.member.admin_user_id, value as number);
          command = SmartSafeCommandCode.SET_VOLUME;
          break;
        case PropertyName.DeviceAlarmVolume:
          payload = SmartSafe.encodeCmdAlertVolume(this.rawStation.member.admin_user_id, value as number);
        ...
        case PropertyName.DeviceNotificationJammed:
          const settingsValues: Array<boolean> = [
            property === PropertyName.DeviceNotificationUnlockByKey
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationUnlockByKey) as boolean),
            property === PropertyName.DeviceNotificationUnlockByPIN
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationUnlockByPIN) as boolean),
            property === PropertyName.DeviceNotificationUnlockByFingerprint
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationUnlockByFingerprint) as boolean),
            property === PropertyName.DeviceNotificationUnlockByApp
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationUnlockByApp) as boolean),
            property === PropertyName.DeviceNotificationDualUnlock
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationDualUnlock) as boolean),
            property === PropertyName.DeviceNotificationDualLock
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationDualLock) as boolean),
            property === PropertyName.DeviceNotificationWrongTryProtect
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationWrongTryProtect) as boolean),
            property === PropertyName.DeviceNotificationJammed
              ? (value as boolean)
              : (device.getPropertyValue(PropertyName.DeviceNotificationJammed) as boolean),
          ];
          ...
          payload = SmartSafe.encodeCmdPushNotification(this.rawStation.member.admin_user_id, modes);
          command = SmartSafeCommandCode.SET_PUSH;
          break;
        case PropertyName.DeviceScramblePasscode:
          payload = SmartSafe.encodeCmdScramblePIN(this.rawStation.member.admin_user_id, value as boolean);
      ...
      rootHTTPLogger.debug(`Station set smart safe params - payload`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        property: property,
        value: value,
  ...
  public unlock(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station unlock - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      payload: payload.toString("hex"),
    });
  ...
  public verifyPIN(device: Device, pin: string): void {
    ...
    rootHTTPLogger.debug(`Station verify pin - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      payload: payload.toString("hex"),
    });
  ...
  public setVideoTypeStoreToNAS(device: Device, value: VideoTypeStoreToNAS): void {
    ...
    if (
      device.getPropertyValue(PropertyName.DeviceContinuousRecording) !== true &&
      value === VideoTypeStoreToNAS.ContinuousRecording
    ) {
      this.setContinuousRecording(device, true);
    }

    rootHTTPLogger.debug(`Station set video type store to nas - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public snooze(device: Device, value: SnoozeDetail): void {
    ...
    rootHTTPLogger.debug(`Station snooze - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public addUser(device: Device, username: string, shortUserId: string, passcode: string, schedule?: Schedule): void {
    ...
    rootHTTPLogger.debug(`Station add user  - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      username: username,
      shortUserId: shortUserId,
    ...
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      const nestedPayload = {
        ...
        startDay:
          schedule !== undefined && schedule.startDateTime !== undefined ? hexDate(schedule.startDateTime) : "00000000",
        startTime:
      ...
      rootHTTPLogger.debug("Station smart lock device - Add user...", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command.payload,
  ...
  public deleteUser(device: Device, username: string, shortUserId: string): void {
    ...
    rootHTTPLogger.debug(`Station delete user - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      username: username,
      shortUserId: shortUserId,
    ...
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      rootHTTPLogger.debug("Station delete user - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
      ...
      const command = getSmartLockP2PCommand(
        ...
        device.getChannel(),
        this.p2pSession.incLockSequenceNumber(),
        Lock.encodeCmdSmartLockDeleteUser(this.rawStation.member.admin_user_id, shortUserId)
      );
      rootHTTPLogger.debug("Station delete user - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
  ...
  public updateUserSchedule(device: Device, username: string, shortUserId: string, schedule: Schedule): void {
    ...
    rootHTTPLogger.debug(`Station update user schedule - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      username: username,
      shortUserId: shortUserId,
    ...
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      rootHTTPLogger.debug("Station update user schedule - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
      ...
      const command = getSmartLockP2PCommand(
        ...
        device.getChannel(),
        this.p2pSession.incLockSequenceNumber(),
        Lock.encodeCmdSmartLockUpdateSchedule(this.rawStation.member.admin_user_id, shortUserId, username, schedule)
      );
      rootHTTPLogger.debug("Station update user schedule - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
  ...
  public updateUserPasscode(device: Device, username: string, passwordId: string, passcode: string): void {
    ...
    rootHTTPLogger.debug(`Station update user passcode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      username: username,
      passwordId: passwordId,
    ...
    if (device.isLockWifi() || device.isLockWifiNoFinger()) {
      ...
      rootHTTPLogger.debug("Station update user passcode - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
  ...
  public setLockV12Params(device: Device, property: PropertyName, value: PropertyValue): void {
    ...
    rootHTTPLogger.debug(`Station set lock v12 settings - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      property: property,
      value: value,
    ...
    if (device.isLockWifiR10() || device.isLockWifiR20()) {
      ...
      switch (property) {
        ...
        case PropertyName.DeviceAutoLockScheduleEndTime:
          ...
          break;
        case PropertyName.DeviceOneTouchLocking:
          payload = Lock.encodeCmdSetLockParamOneTouchLock(value as boolean);
          break;
        case PropertyName.DeviceScramblePasscode:
          payload = Lock.encodeCmdSetLockParamScramblePasscode(value as boolean);
          break;
        case PropertyName.DeviceSound:
          payload = Lock.encodeCmdSetLockParamSound(value as number);
          break;
        default:
          payload = Buffer.from([]);
          break;
      ...
      rootHTTPLogger.debug(`Station set lock v12 settings - payload`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        property: property,
        value: value,
  ...
  public setSmartLockParams(device: Device, property: PropertyName, value: PropertyValue): void {
    ...
    rootHTTPLogger.debug(`Station set smart lock settings - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      property: property,
      value: value,
    ...
    if (
      ...
      switch (property) {
        ...
        case PropertyName.DeviceAutoLockScheduleEndTime:
          ...
          break;
        case PropertyName.DeviceOneTouchLocking:
          payload = Lock.encodeCmdSetSmartLockParamOneTouchLock(this.rawStation.member.admin_user_id, value as boolean);
          break;
        case PropertyName.DeviceScramblePasscode:
          ...
          break;
        case PropertyName.DeviceSound:
          payload = Lock.encodeCmdSetSmartLockParamSound(this.rawStation.member.admin_user_id, value as number);
          break;
        default:
          payload = Buffer.from([]);
          break;
      ...
      rootHTTPLogger.debug(`Station set smart lock settings - payload`, {
        stationSN: this.getSerial(),
        deviceSN: device.getSerial(),
        property: property,
        value: value,
  ...
  public setAutoLock(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set auto lock - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAutoLockSchedule(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set auto lock schedule - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAutoLockScheduleStartTime(device: Device, value: string): void {
    ...
    rootHTTPLogger.debug(`Station set auto lock schedule start time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAutoLockScheduleEndTime(device: Device, value: string): void {
    ...
    rootHTTPLogger.debug(`Station set auto lock schedule end time - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setAutoLockTimer(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set auto lock timer - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setOneTouchLocking(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set one touch locking - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setSound(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set sound - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotification(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationLocked(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification locked - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNotificationUnlocked(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set notification unlocked - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public queryAllUserId(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station query all user id - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    if (device.isLockBleNoFinger() || device.isLockBle()) {
      throw new NotSupportedError("This functionality is not implemented by this device", {
        context: { device: device.getSerial(), station: this.getSerial(), commandName: commandData.name },
      });
      ...
      rootHTTPLogger.debug("Querying all user id...", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
      ...
      rootHTTPLogger.debug("Station get user list - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command.payload,
  ...
  public chimeHomebase(value: number): void {
      ...
      this.rawStation.devices.forEach((device) => {
        if (Device.isDoorbell(device.device_type)) {
          throw new NotSupportedError(
            "This functionality is only supported on stations without registered Doorbells on it",
            { context: { commandName: commandData.name, station: this.getSerial() } }
          );
        }
      });
    ...
    if (this.isStation()) {
      ...
      throw new NotSupportedError("This functionality is not implemented or supported", {
        context: { commandName: commandData.name, commandValue: commandData.value, station: this.getSerial() },
      });
    }
  }
  ...
  public downloadImage(cover_path: string): void {
    ...
    rootHTTPLogger.debug(`Station download image - sending command`, {
      stationSN: this.getSerial(),
      value: cover_path,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public databaseQueryLocal(
    ...
    rootHTTPLogger.debug(`Station database query local - sending command`, {
      stationSN: this.getSerial(),
      serialNumbers: serialNumbers,
      startDate: startDate,
      endDate: endDate,
  ...
  public databaseQueryByDate(
    ...
    rootHTTPLogger.debug(`Station database query by date - sending command`, {
      stationSN: this.getSerial(),
      serialNumbers: serialNumbers,
      startDate: startDate,
      endDate: endDate,
  ...
  public databaseCountByDate(startDate: Date, endDate: Date): void {
    ...
    rootHTTPLogger.debug(`Station database count by date - sending command`, {
      stationSN: this.getSerial(),
      startDate: startDate,
      endDate: endDate,
    });
  ...
  public setMotionDetectionTypeHuman(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection type human - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMotionDetectionTypeAllOtherMotions(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion detection type all other motions - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  private _setLightSettingsLightingActiveMode(
    device: Device,
    propertyName: PropertyName,
    value: LightingActiveMode,
    type: "manual" | "schedule" | "motion"
  ): void {
    const propertyData: PropertyData = {
      name: propertyName,
      value: value,
    ...
    rootHTTPLogger.debug(`Station set set light settings lighting active mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyName: propertyName,
      value: value,
    ...
    if (device.isWallLightCam()) {
      switch (value) {
          ...
          this.p2pSession.sendCommandWithStringPayload(
            {
              ...
              value: JSON.stringify({
                commandType:
                  type === "manual"
                    ? CommandType.CMD_WALL_LIGHT_SETTINGS_MANUAL_DAILY_LIGHTING
                    : type === "schedule"
                      ? CommandType.CMD_WALL_LIGHT_SETTINGS_SCHEDULE_DAILY_LIGHTING
                      : CommandType.CMD_WALL_LIGHT_SETTINGS_MOTION_DAILY_LIGHTING,
        ...
        case LightingActiveMode.COLORED: {
          ...
          const colors: Array<RGBColor> = device.getPropertyValue(
            PropertyName.DeviceLightSettingsColoredLightingColors
          ) as Array<RGBColor>;
          if (
            !colors.some(
              (color) =>
                color.red === currentPropertyValue.red &&
                color.green === currentPropertyValue.green &&
                color.blue === currentPropertyValue.blue
            )
          ) {
                ...
                commandType:
                  type === "manual"
                    ...
                    : type === "schedule"
                      ...
                      : CommandType.CMD_WALL_LIGHT_SETTINGS_MOTION_COLORED_LIGHTING,
                data: {
        ...
        case LightingActiveMode.DYNAMIC: {
          ...
          let currentPropertyValue = device.getPropertyValue(currentProperty) as number;
          const range: number = (
            device.getPropertyValue(PropertyName.DeviceLightSettingsDynamicLightingThemes) as Array<DynamicLighting>
          ).length;
              ...
              value: JSON.stringify({
                commandType:
                  type === "manual"
                    ...
                    : type === "schedule"
                      ? CommandType.CMD_WALL_LIGHT_SETTINGS_SCHEDULE_DYNAMIC_LIGHTING
                      : CommandType.CMD_WALL_LIGHT_SETTINGS_MOTION_DYNAMIC_LIGHTING,
                data: currentPropertyValue,
              }),
              channel: device.getChannel(),
            },
  ...
  public setLightSettingsManualLightingActiveMode(device: Device, value: LightingActiveMode): void {
    this._setLightSettingsLightingActiveMode(
      device,
  ...
  public setLightSettingsManualDailyLighting(device: Device, value: DailyLightingType): void {
    ...
    rootHTTPLogger.debug(`Station set light settings manual daily lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsManualColoredLighting(device: Device, value: RGBColor): void {
    ...
    rootHTTPLogger.debug(`Station set light settings manual colored lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsManualDynamicLighting(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings manual dynamic lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsMotionDailyLighting(device: Device, value: DailyLightingType): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion daily lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsMotionColoredLighting(device: Device, value: RGBColor): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion colored lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsMotionDynamicLighting(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings motion dynamic lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsScheduleDailyLighting(device: Device, value: DailyLightingType): void {
    ...
    rootHTTPLogger.debug(`Station set light settings schedule daily lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsScheduleColoredLighting(device: Device, value: RGBColor): void {
    ...
    rootHTTPLogger.debug(`Station set light settings schedule colored lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsScheduleDynamicLighting(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set light settings schedule dynamic lighting - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setLightSettingsColoredLightingColors(device: Device, value: Array<RGBColor>): void {
    ...
    rootHTTPLogger.debug(`Station set light settings colored lighting colors - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isWallLightCam()) {
      const colors: Array<InternalColoredLighting> = [
        { color: 16760832 },
        { color: 16744448 },
        { color: 16728320 },
        { color: 16720384 },
      ...
      if (value.length > 0 && value.length <= 15) {
        ...
        if (value.length - count + colors.length > 15) {
          throw new InvalidPropertyValueError(
            "This property can contain a maximum of 15 items, of which the first 10 are fixed. You can either deliver the first 10 static items with the maximum 5 freely selectable items or only the maximum 5 freely selectable items.",
            {
              context: {
                device: device.getSerial(),
                station: this.getSerial(),
  ...
  public setLightSettingsDynamicLightingThemes(device: Device, value: Array<DynamicLighting>): void {
    ...
    rootHTTPLogger.debug(`Station set light settings dynamic lighting themes - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
    if (device.isWallLightCam()) {
      const themes: Array<InternalDynamicLighting> = [
        { name: "Aurora", mode: 1, id: 0, speed: 4000, colors: [65321, 65468, 28671, 9215, 42239] },
        { name: "Warmth", mode: 1, id: 1, speed: 4000, colors: [16758528, 16744448, 16732160, 16719360, 16742144] },
        { name: "Let's Party", mode: 2, id: 2, speed: 500, colors: [16718080, 16756736, 65298, 40703, 4980991] },
      ];
      if (value.length > 0 && value.length <= 23) {
        ...
        if (value.length - count + themes.length > 23) {
          throw new InvalidPropertyValueError(
            "This property can contain a maximum of 23 items, of which the first 3 are fixed. You can either deliver the first 3 static items with the maximum 20 freely selectable items or only the maximum 20 freely selectable items.",
            {
              context: {
                device: device.getSerial(),
                station: this.getSerial(),
  ...
  public setDoorControlWarning(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set door control warning - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public openDoor(device: Device, value: boolean, doorId = 1): void {
    ...
    rootHTTPLogger.debug(`Station open door - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
      doorId: doorId,
  ...
  public calibrateGarageDoor(device: Device, doorId: number, type: CalibrateGarageType): void {
    ...
    rootHTTPLogger.debug(`Station calibrate garage door  - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      doorId: doorId,
      type: type,
  ...
  public setMirrorMode(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set mirror mode - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setFlickerAdjustment(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set flicker adjustment - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setCrossCameraTracking(value: boolean): void {
    ...
    if (!this.hasProperty(propertyData.name)) {
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { station: this.getSerial(), propertyName: propertyData.name, propertyValue: propertyData.value },
      });
    }
    const property = this.getPropertyMetadata(propertyData.name);
    validValue(property, value);

    rootHTTPLogger.debug(`Station set cross camera tracking - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setContinuousTrackingTime(value: number): void {
    ...
    const property = this.getPropertyMetadata(propertyData.name);
    validValue(property, value);
    rootHTTPLogger.debug(`Station set continuous tracking time - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setTrackingAssistance(value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set tracking assistance - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setCrossTrackingCameraList(value: Array<string>): void {
    ...
    rootHTTPLogger.debug(`Station set cross tracking camera list - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setCrossTrackingGroupList(value: Array<CrossTrackingGroupEntry>): void {
    ...
    rootHTTPLogger.debug(`Station set cross tracking group list - sending command`, {
      stationSN: this.getSerial(),
      value: value,
    });
    this.p2pSession.sendCommandWithStringPayload(
  ...
  public setNotificationIndoor(device: Device, type: IndoorS350NotificationTypes, value: boolean): void {
    const propertyData: PropertyData = {
      name:
        type === IndoorS350NotificationTypes.ALL_OTHER_MOTION
          ? PropertyName.DeviceNotificationAllOtherMotion
          : type === IndoorS350NotificationTypes.HUMAN
            ? PropertyName.DeviceNotificationPerson
            : type === IndoorS350NotificationTypes.PET
              ? PropertyName.DeviceNotificationPet
              : type === IndoorS350NotificationTypes.CRYING
    ...
    rootHTTPLogger.debug(`Station set notification type indoor - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      type: type,
      value: value,
      ...
      try {
        const notification =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        let newNotification = getIndoorNotification(
          Number.parseInt(notification),
          type as IndoorS350NotificationTypes,
          value
        );
        ...
        this.p2pSession.sendCommandWithStringPayload(
          ...
          },
          {
            ...
            onSuccess: () => {
              device.updateRawProperty(
                CommandType.CMD_INDOOR_SET_MOTION_DETECTION_TYPE,
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setNotificationIndoor Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
  ...
  public setNotificationFloodlightT8425(device: Device, type: FloodlightT8425NotificationTypes, value: boolean): void {
    const propertyData: PropertyData = {
      name:
        type === FloodlightT8425NotificationTypes.ALL_OTHER_MOTION
          ? PropertyName.DeviceNotificationAllOtherMotion
          : type === FloodlightT8425NotificationTypes.HUMAN
            ? PropertyName.DeviceNotificationPerson
            : type === FloodlightT8425NotificationTypes.PET
              ? PropertyName.DeviceNotificationPet
    ...
    rootHTTPLogger.debug(`Station set notification type floodlight T8425 - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      type: type,
      value: value,
      ...
      try {
        const notification =
          device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number) !== undefined
            ? device.getRawProperty(device.getPropertyMetadata(propertyData.name).key as number)!
            : "0";
        let newNotification = getFloodLightT8425Notification(
          Number.parseInt(notification),
          type as FloodlightT8425NotificationTypes,
          value
        );
        ...
        const error = ensureError(err);
        rootHTTPLogger.error(`setNotificationFloodlightT8425 Error`, {
          error: getError(error),
          stationSN: this.getSerial(),
          deviceSN: device.getSerial(),
        });
  ...
  public presetPosition(device: Device, position: PresetPositionType): void {
    ...
    rootHTTPLogger.debug(`Station preset position - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      preset: PresetPositionType[position],
    });
    if (
      device.isFloodLightT8425() ||
      device.isIndoorPanAndTiltCameraS350() ||
  ...
  public savePresetPosition(device: Device, position: PresetPositionType): void {
    ...
    rootHTTPLogger.debug(`Station save preset position - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      preset: PresetPositionType[position],
    });
  ...
  public deletePresetPosition(device: Device, position: PresetPositionType): void {
    ...
    rootHTTPLogger.debug(`Station delete preset position - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      preset: PresetPositionType[position],
    });
  ...
  public setLeavingDetection(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set leaving detection - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  private _setLeavingReactions(
    ...
    rootHTTPLogger.debug(`Station set leaving reactions - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      propertyData: propertyData,
      pushNotification: pushNotification,
  ...
  public setBeepVolume(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set beep volume - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setNightvisionOptimization(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set nightvision optimization - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public getLockParameters(): void {
    ...
    if (
      ...
      Device.isLockWifiT85D0(this.getDeviceType())
    ) {
  ...
  public getLockStatus(): void {
    ...
    if (
      ...
      Device.isLockWifi(this.getDeviceType(), this.getSerial()) ||
      Device.isLockWifiNoFinger(this.getDeviceType())
    ) {
      this.p2pSession.sendCommandWithStringPayload({
        commandType: CommandType.CMD_SET_PAYLOAD,
        value: JSON.stringify({
          ...
          payload: {
            timezone:
              this.rawStation.time_zone === undefined || this.rawStation.time_zone === ""
                ? getAdvancedLockTimezone(this.rawStation.station_sn)
                : this.rawStation.time_zone,
          },
        }),
        channel: 0,
      } as P2PCommand);
  ...
  private onSequenceError(channel: number, command: number, sequence: number, serialnumber: string): void {
    ...
    rootHTTPLogger.debug(`Station lock sequence error`, {
      stationSN: this.getSerial(),
      channel: channel,
      command: command,
      sequence: sequence,
  ...
  public updateUsername(device: Device, username: string, passwordId: string): void {
    ...
    rootHTTPLogger.debug(`Station update username - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      username: username,
      passwordId: passwordId,
    ...
    if (device.isLockWifiR10() || device.isLockWifiR20()) {
      ...
      rootHTTPLogger.debug("Station update username - payload", {
        station: this.getSerial(),
        device: device.getSerial(),
        admin_user_id: this.rawStation.member.admin_user_id,
        payload: command,
  ...
  public setOpenMethod(device: Device, value: number): void {
    ...
    rootHTTPLogger.debug(`Station set open method - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public setMotionActivatedPrompt(device: Device, value: boolean): void {
    ...
    rootHTTPLogger.debug(`Station set motion activated prompt - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
      value: value,
    });
  ...
  public open(device: Device): void {
    ...
    rootHTTPLogger.debug(`Station open - sending command`, {
      stationSN: this.getSerial(),
      deviceSN: device.getSerial(),
    });
    if (device.isSmartDrop()) {
