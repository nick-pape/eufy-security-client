...
export class EufySecurity extends TypedEmitter<EufySecurityEvents> {
    ...
    try {
      if (this.persistentData.version !== libVersion) {
        ...
        const previousVersion =
          this.persistentData.version !== "" && this.persistentData.version !== undefined
            ? Number.parseFloat(removeLastChar(this.persistentData.version, "."))
            : 0;
  ...
  private async updateStation(hub: StationListResponse): Promise<void> {
    ...
    if (Object.keys(this.stations).includes(hub.station_sn)) {
      ...
      if (
        ...
        rootMainLogger.debug(`Updating station cloud data - initiate station connection to get local data over p2p`, {
  ...
  private async updateDevice(device: DeviceListResponse): Promise<void> {
    if (this.devicesLoaded) await this.devicesLoaded;
    if (Object.keys(this.devices).includes(device.device_sn)) this.devices[device.device_sn].update(device);
    else rootMainLogger.debug(`Device with this serial ${device.device_sn} doesn't exists and couldn't be updated!`);
  }
  ...
  public async getDevicesFromStation(stationSN: string): Promise<Array<Device>> {
    ...
    const arr: Array<Device> = [];
    Object.keys(this.devices).forEach((serialNumber: string) => {
      if (this.devices[serialNumber].getStationSerial() === stationSN) arr.push(this.devices[serialNumber]);
    });
    return arr;
  }
  ...
  public async getDevice(deviceSN: string): Promise<Device> {
    if (this.devicesLoaded) await this.devicesLoaded;
    if (Object.keys(this.devices).includes(deviceSN)) return this.devices[deviceSN];
  ...
  public async getStationDevice(stationSN: string, channel: number): Promise<Device> {
    ...
    for (const device of Object.values(this.devices)) {
      if (
        (device.getStationSerial() === stationSN && device.getChannel() === channel) ||
        (device.getStationSerial() === stationSN && device.getSerial() === stationSN)
      ) {
        return device;
      }
    }
    throw new DeviceNotFoundError("No device with passed channel found on station", {
  ...
  public async getStation(stationSN: string): Promise<Station> {
    if (this.stationsLoaded) await this.stationsLoaded;
    if (Object.keys(this.stations).includes(stationSN)) return this.stations[stationSN];
        ...
        promises.push(
          station.then((station: Station) => {
            try {
              ...
              station.on("raw device property changed", (deviceSN: string, params: RawValues) =>
                this.updateDeviceProperties(deviceSN, params)
              );
              station.on(
                "livestream start",
                (
              ...
              station.on("download finish", (station: Station, channel: number) =>
                this.onStationFinishDownload(station, channel)
              );
              ...
              station.on("current mode", (station: Station, currentMode: number) =>
                this.onStationCurrentMode(station, currentMode)
              );
              station.on("rtsp livestream start", (station: Station, channel: number) =>
                this.onStartStationRTSPLivestream(station, channel)
              );
              station.on("rtsp livestream stop", (station: Station, channel: number) =>
                this.onStopStationRTSPLivestream(station, channel)
              );
              station.on("rtsp url", (station: Station, channel: number, value: string) =>
                this.onStationRtspUrl(station, channel, value)
              );
              station.on("property changed", (station: Station, name: string, value: PropertyValue, ready: boolean) =>
                this.onStationPropertyChanged(station, name, value, ready)
              );
              station.on("raw property changed", (station: Station, type: number, value: string) =>
                this.onStationRawPropertyChanged(station, type, value)
              );
              station.on("alarm event", (station: Station, alarmEvent: AlarmEvent) =>
                this.onStationAlarmEvent(station, alarmEvent)
              );
              station.on(
                "runtime state",
                (station: Station, channel: number, batteryLevel: number, temperature: number) =>
                  this.onStationRuntimeState(station, channel, batteryLevel, temperature)
              );
              station.on(
                "charging state",
                (station: Station, channel: number, chargeType: number, batteryLevel: number) =>
                  this.onStationChargingState(station, channel, chargeType, batteryLevel)
              );
              station.on("wifi rssi", (station: Station, channel: number, rssi: number) =>
                this.onStationWifiRssi(station, channel, rssi)
              );
              station.on("floodlight manual switch", (station: Station, channel: number, enabled: boolean) =>
                this.onFloodlightManualSwitch(station, channel, enabled)
              );
              ...
              station.on("talkback started", (station: Station, channel: number, talkbackStream: TalkbackStream) =>
                this.onStationTalkbackStart(station, channel, talkbackStream)
              );
              station.on("talkback stopped", (station: Station, channel: number) =>
                this.onStationTalkbackStop(station, channel)
              );
              station.on("talkback error", (station: Station, channel: number, error: Error) =>
                this.onStationTalkbackError(station, channel, error)
              );
              ...
              station.on("alarm arm delay event", (station: Station, armDelay: number) =>
                this.onStationArmDelayEvent(station, armDelay)
              );
              station.on("secondary command result", (station: Station, result: CommandResult) =>
                this.onStationSecondaryCommandResult(station, result)
              );
              station.on("device shake alarm", (deviceSN: string, event: SmartSafeShakeAlarmEvent) =>
                this.onStationDeviceShakeAlarm(deviceSN, event)
              );
              station.on("device 911 alarm", (deviceSN: string, event: SmartSafeAlarm911Event) =>
                this.onStationDevice911Alarm(deviceSN, event)
              );
              ...
              station.on(
                "sd info ex",
                (station: Station, sdStatus: TFCardStatus, sdCapacity: number, sdCapacityAvailable: number) =>
                  this.onStationSdInfoEx(station, sdStatus, sdCapacity, sdCapacityAvailable)
              );
              station.on("image download", (station: Station, file: string, image: Buffer) =>
                this.onStationImageDownload(station, file, image)
              );
              station.on(
              ...
              station.on(
                "database delete",
                (station: Station, returnCode: DatabaseReturnCode, failedIds: Array<unknown>) =>
                  this.onStationDatabaseDelete(station, returnCode, failedIds)
              );
              station.on("sensor status", (station: Station, channel: number, status: number) =>
                this.onStationSensorStatus(station, channel, status)
              );
              station.on("garage door status", (station: Station, channel: number, doorId: number, status: number) =>
                this.onStationGarageDoorStatus(station, channel, doorId, status)
              );
  ...
  private refreshP2PData(station: Station): void {
    if (
      ...
      Device.isSmartSafe(station.getDeviceType())
    ) {
    ...
    if (
      station.isStation() ||
      (station.hasProperty(PropertyName.StationSdStatus) &&
        station.getPropertyValue(PropertyName.StationSdStatus) !== TFCardStatus.REMOVE)
    ) {
          ...
          new_device.then((device: Device) => {
            try {
              device.on("property changed", (device: Device, name: string, value: PropertyValue, ready: boolean) =>
                this.onDevicePropertyChanged(device, name, value, ready)
              );
              device.on("raw property changed", (device: Device, type: number, value: string) =>
                this.onDeviceRawPropertyChanged(device, type, value)
              );
              ...
              device.on("person detected", (device: Device, state: boolean, person: string) =>
                this.onDevicePersonDetected(device, state, person)
              );
              ...
              device.on("radar motion detected", (device: Device, state: boolean) =>
                this.onDeviceRadarMotionDetected(device, state)
              );
              ...
              device.on("shake alarm", (device: Device, state: boolean, detail: SmartSafeShakeAlarmEvent) =>
                this.onDeviceShakeAlarm(device, state, detail)
              );
              device.on("wrong try-protect alarm", (device: Device, state: boolean) =>
                this.onDeviceWrongTryProtectAlarm(device, state)
              );
              ...
              device.on("stranger person detected", (device: Device, state: boolean) =>
                this.onDeviceStrangerPersonDetected(device, state)
              );
              ...
              device.on("battery fully charged", (device: Device, state: boolean) =>
                this.onDeviceBatteryFullyCharged(device, state)
              );
    ...
    Promise.all(promises).then((devices) => {
      devices.forEach((device) => {
        this.getStation(device.getStationSerial())
          ...
          .catch((err) => {
            const error = ensureError(err);
            rootMainLogger.error("Error trying to connect to station afte device loaded", {
  ...
  public async registerPushNotifications(credentials?: Credentials, persistentIds?: string[]): Promise<void> {
    if (credentials) this.pushService.setCredentials(credentials);
    if (persistentIds) this.pushService.setPersistentIds(persistentIds);
    ...
    await this.api
      ...
      .then(async () => {
        if (options?.verifyCode) {
          ...
          if (!trusted) return await this.api.addTrustDevice(options?.verifyCode);
        }
      })
      .catch((err) => {
        const error = ensureError(err);
  ...
  private async onAPIClose(): Promise<void> {
    ...
    if (this.retries < 3) {
      ...
      rootMainLogger.error(
        `Tried to re-authenticate to Eufy cloud, but failed in the process. Manual intervention is required!`
      );
    }
  }
  ...
  public async startStationLivestream(deviceSN: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceStartLivestream))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceStartLivestream },
      });
    ...
    if (!station.isLiveStreaming(camera)) {
      ...
      rootMainLogger.warn(
        `The station stream for the device ${deviceSN} cannot be started, because it is already streaming!`
      );
    }
  }
  ...
  private async processInvitations(): Promise<void> {
    ...
    if (Object.keys(invites).length > 0) {
      ...
      if (confirmInvites.length > 0) {
        const result = await this.api.confirmInvites(confirmInvites).catch((err) => {
          const error = ensureError(err);
          rootMainLogger.error("Error in confirmation of invitations", {
            error: getError(error),
    ...
    if (Object.keys(houseInvites).length > 0) {
      for (const invite of Object.values(houseInvites) as HouseInviteListResponse[]) {
        ...
        if (result) {
          rootMainLogger.info(`Accepted received house invitation from ${invite.action_user_email}`, {
            invite: invite,
          });
          refreshCloud = true;
        }
  ...
  private onPushMessage(message: PushMessage): void {
    ...
    try {
      ...
      try {
        if (
          (message.type === ServerPushEvent.INVITE_DEVICE || message.type === ServerPushEvent.HOUSE_INVITE) &&
          this.config.acceptInvitations
        ) {
          if (this.isConnected()) this.processInvitations();
        }
      } catch (err) {
        const error = ensureError(err);
        rootMainLogger.error(`Error processing server push notification for device invitation`, {
          error: getError(error),
          message: message,
        });
      }
      try {
        if (
          message.type === ServerPushEvent.REMOVE_DEVICE ||
          message.type === ServerPushEvent.REMOVE_HOMEBASE ||
          message.type === ServerPushEvent.HOUSE_REMOVE
        ) {
          if (this.isConnected()) this.refreshCloudData();
        }
      } catch (err) {
        const error = ensureError(err);
        rootMainLogger.error(`Error processing server push notification for device/station/house removal`, {
          error: getError(error),
          message: message,
        });
      }
            ...
            this.getStation(device.getStationSerial())
              .then((station) => {
                try {
                  ...
                  const error = ensureError(err);
                  rootMainLogger.error(`Error processing push notification for device`, {
                    error: getError(error),
                    deviceSN: device.getSerial(),
                    message: message,
                  });
  ...
  public async startStationDownload(deviceSN: string, path: string, cipherID: number): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceStartDownload))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceStartDownload, path: path, cipherID: cipherID },
      });
  ...
  public async cancelStationDownload(deviceSN: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceCancelDownload))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceCancelDownload },
      });
  ...
  public async setDeviceProperty(deviceSN: string, name: string, value: unknown): Promise<void> {
    ...
    switch (name) {
      ...
      default:
        ...
        throw new InvalidPropertyError("Device has no writable property", {
          context: { device: deviceSN, propertyName: name, propertyValue: value },
        });
    }
  }
  ...
  private onStartStationLivestream(
    ...
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station start livestream error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          metadata: metadata,
  ...
  private onStopStationLivestream(station: Station, channel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station stop livestream error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onErrorStationLivestream(station: Station, channel: number, origError: Error): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station livestream error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          origError: getError(origError),
        });
      });
  }
  ...
  private onStartStationRTSPLivestream(station: Station, channel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station start rtsp livestream error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onStopStationRTSPLivestream(station: Station, channel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station stop rtsp livestream error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onStationStartDownload(
    ...
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station start download error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          metadata: metadata,
  ...
  private onStationFinishDownload(station: Station, channel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station finish download error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
    ...
    if (result.return_code === 0) {
      if (result.customData !== undefined && result.customData.onSuccess !== undefined) {
        try {
          ...
          const error = ensureError(err);
          rootMainLogger.error(`Station command result - onSuccess callback error`, {
            error: getError(error),
            stationSN: station.getSerial(),
            result: result,
          });
      ...
      this.getStationDevice(station.getSerial(), result.channel)
        .then((device: Device) => {
          if (
            ...
            result.customData.command.name === CommandName.DeviceSnooze
          ) {
            const snoozeTime =
              result.customData.command.value !== undefined && result.customData.command.value.snooze_time !== undefined
                ? (result.customData.command.value.snooze_time as number)
                : 0;
            if (snoozeTime > 0) {
      ...
      if (
        station.isIntegratedDevice() &&
        result.command_type === CommandType.CMD_SET_ARMING &&
        station.isConnected() &&
        station.getDeviceType() !== DeviceType.DOORBELL
      ) {
        station.getCameraInfo();
      }
    } else {
      if (result.customData !== undefined && result.customData.onFailure !== undefined) {
        try {
          ...
          const error = ensureError(err);
          rootMainLogger.error(`Station command result - onFailure callback error`, {
            error: getError(error),
            stationSN: station.getSerial(),
            result: result,
          });
            ...
            switch (result.return_code) {
              ...
              case 4:
                this.emit(
                  ...
                  new AddUserError("Passcode already used by another user, please choose a different one", {
                    context: {
                  ...
                  .then((result) => {
                    if (result) {
                      ...
                      this.emit(
                        ...
                        device,
                        customValue.username,
                        new DeleteUserError("Error in deleting user through cloud api call", {
                          context: {
                            device: device.getSerial(),
                            username: customValue.username,
            ...
            switch (result.return_code) {
              ...
              default:
                this.emit(
                  ...
                  device,
                  customValue.username,
                  new UpdateUserPasscodeError("Error updating user passcode", {
                    context: {
                      device: device.getSerial(),
                      username: customValue.username,
                      returnCode: result.return_code,
            ...
            switch (result.return_code) {
              ...
              default:
                this.emit(
                  ...
                  device,
                  customValue.username,
                  new UpdateUserScheduleError("Error updating user schedule", {
                    context: {
                      device: device.getSerial(),
                      username: customValue.username,
                      schedule: customValue.schedule,
  ...
  private onStationRtspUrl(station: Station, channel: number, value: string): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station rtsp url error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          url: value,
    ...
    try {
      ...
      if (
        ...
        (device.getPropertyValue(PropertyName.DeviceRTSPStreamUrl) === undefined ||
          (device.getPropertyValue(PropertyName.DeviceRTSPStreamUrl) !== undefined &&
            (device.getPropertyValue(PropertyName.DeviceRTSPStreamUrl) as string) === ""))
      ) {
        this.getStation(device.getStationSerial())
          .then((station: Station) => {
          ...
          .catch((err) => {
            const error = ensureError(err);
            rootMainLogger.error(`Device property changed error - station enable rtsp`, {
              error: getError(error),
              deviceSN: device.getSerial(),
      ...
      const error = ensureError(err);
      rootMainLogger.error(`Device property changed error`, {
        error: getError(error),
        deviceSN: device.getSerial(),
        stationSN: device.getStationSerial(),
        propertyName: name,
  ...
  private onDeviceReady(device: Device): void {
    try {
      if (
        ...
        (device.getPropertyValue(PropertyName.DeviceRTSPStream) as boolean) === true
      ) {
        this.getStation(device.getStationSerial())
          .then((station: Station) => {
            station.setRTSPStream(device, true);
          ...
          .catch((err) => {
            const error = ensureError(err);
            rootMainLogger.error(`Device ready error - station enable rtsp`, {
              error: getError(error),
              deviceSN: device.getSerial(),
              stationSN: device.getStationSerial(),
            });
      ...
      const error = ensureError(err);
      rootMainLogger.error(`Device ready error`, {
        error: getError(error),
        deviceSN: device.getSerial(),
        stationSN: device.getStationSerial(),
      });
  ...
  private onStationRuntimeState(station: Station, channel: number, batteryLevel: number, temperature: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station runtime state error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
  ...
  private onStationChargingState(station: Station, channel: number, chargeType: number, batteryLevel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station charging state error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
  ...
  private onStationWifiRssi(station: Station, channel: number, rssi: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station wifi rssi error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          rssi: rssi,
  ...
  private onStationTalkbackStart(station: Station, channel: number, talkbackStream: TalkbackStream): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station talkback start error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onStationTalkbackStop(station: Station, channel: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station talkback stop error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onStationTalkbackError(station: Station, channel: number, origError: Error): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station talkback error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
          origError: getError(origError),
  ...
  public async startStationTalkback(deviceSN: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceStartTalkback))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceStartTalkback },
      });
    ...
    if (station.isLiveStreaming(device)) {
      if (!station.isTalkbackOngoing(device)) {
        ...
        rootMainLogger.warn(
          `The station talkback for the device ${deviceSN} cannot be started, because it is ongoing!`
        );
      }
    } else {
      rootMainLogger.warn(
        `The station talkback for the device ${deviceSN} cannot be started, because it isn't live streaming!`
      );
    }
  }
  ...
  public async stopStationTalkback(deviceSN: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceStopTalkback))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceStopTalkback },
      });
    ...
    if (station.isLiveStreaming(device)) {
      if (station.isTalkbackOngoing(device)) {
        ...
        rootMainLogger.warn(
          `The station talkback for the device ${deviceSN} cannot be stopped, because it isn't ongoing!`
        );
      }
    } else {
      rootMainLogger.warn(
        `The station talkback for the device ${deviceSN} cannot be stopped, because it isn't live streaming!`
      );
    }
  }
  ...
  private onStationDeviceShakeAlarm(deviceSN: string, event: SmartSafeShakeAlarmEvent): void {
    this.getDevice(deviceSN)
      .then((device: Device) => {
        if (device.isSmartSafe()) (device as SmartSafe).shakeEvent(event, this.config.eventDurationSeconds);
      })
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`onStationDeviceShakeAlarm error`, {
          error: getError(error),
          deviceSN: deviceSN,
          event: SmartSafeShakeAlarmEvent[event],
        });
      });
  }
  ...
  private onStationDevice911Alarm(deviceSN: string, event: SmartSafeAlarm911Event): void {
    this.getDevice(deviceSN)
      .then((device: Device) => {
        if (device.isSmartSafe()) (device as SmartSafe).alarm911Event(event, this.config.eventDurationSeconds);
      })
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`onStationDevice911Alarm error`, {
          error: getError(error),
          deviceSN: deviceSN,
          event: SmartSafeAlarm911Event[event],
        });
      });
  }
  ...
  private onStationDeviceJammed(deviceSN: string): void {
    this.getDevice(deviceSN)
      .then((device: Device) => {
        if (device.isSmartSafe()) (device as SmartSafe).jammedEvent(this.config.eventDurationSeconds);
      })
      .catch((err) => {
        const error = ensureError(err);
  ...
  private onStationDeviceLowBattery(deviceSN: string): void {
    this.getDevice(deviceSN)
      .then((device: Device) => {
        if (device.isSmartSafe()) (device as SmartSafe).lowBatteryEvent(this.config.eventDurationSeconds);
      })
      .catch((err) => {
        const error = ensureError(err);
  ...
  private onStationDeviceWrongTryProtectAlarm(deviceSN: string): void {
    this.getDevice(deviceSN)
      .then((device: Device) => {
        if (device.isSmartSafe()) (device as SmartSafe).wrongTryProtectAlarmEvent(this.config.eventDurationSeconds);
      })
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`onStationDeviceWrongTryProtectAlarm error`, {
          error: getError(error),
          deviceSN: deviceSN,
        });
      });
    ...
    try {
      ...
      if (addUserResponse !== null) {
        ...
        this.emit(
          ...
          device,
          username,
          new AddUserError("Error on creating user through cloud api call", {
      ...
      const error = ensureError(err);
      rootMainLogger.error(`addUser error`, {
        error: getError(error),
        deviceSN: deviceSN,
        username: username,
        schedule: schedule,
  ...
  public async deleteUser(deviceSN: string, username: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceDeleteUser))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: { device: deviceSN, commandName: CommandName.DeviceDeleteUser, username: username },
      });
    ...
    try {
      ...
      if (users !== null) {
        ...
        if (!found) {
          this.emit(
            ...
            device,
            username,
            new DeleteUserError("User not found", { context: { device: deviceSN, username: username } })
          );
        }
      } else {
        this.emit(
          ...
          device,
          username,
          new DeleteUserError("Error on getting user list through cloud api call", {
      ...
      this.emit(
        ...
        device,
        username,
        new DeleteUserError("Generic error", { cause: error, context: { device: deviceSN, username: username } })
      );
    }
  }
  ...
  public async updateUser(deviceSN: string, username: string, newUsername: string): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceUpdateUsername))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: {
          device: deviceSN,
          commandName: CommandName.DeviceUpdateUsername,
          usernmae: username,
    ...
    try {
      ...
      if (users !== null) {
          ...
          if (user.user_name === username) {
            ...
            if (result) {
              ...
              this.emit(
                ...
                device,
                username,
                new UpdateUserUsernameError("Error in changing username through cloud api call", {
        ...
        this.emit(
          ...
          device,
          username,
          new UpdateUserUsernameError("Error on getting user list through cloud api call", {
            context: { device: deviceSN, username: username, newUsername: newUsername },
          })
        );
      }
      ...
      const error = ensureError(err);
      rootMainLogger.error(`updateUser error`, {
        error: getError(error),
        deviceSN: deviceSN,
        username: username,
        newUsername: newUsername,
    ...
    try {
      ...
      if (users !== null) {
        ...
        this.emit(
          ...
          device,
          username,
          new UpdateUserPasscodeError("Error on getting user list through cloud api call", {
            context: { device: deviceSN, username: username, passcode: "[redacted]" },
          })
        );
      }
      ...
      const error = ensureError(err);
      rootMainLogger.error(`updateUserPasscode error`, {
        error: getError(error),
        deviceSN: deviceSN,
        username: username,
      });
  ...
  public async updateUserSchedule(deviceSN: string, username: string, schedule: Schedule): Promise<void> {
    ...
    if (!device.hasCommand(CommandName.DeviceUpdateUserSchedule))
      throw new NotSupportedError("This functionality is not implemented or supported by this device", {
        context: {
          device: deviceSN,
          commandName: CommandName.DeviceUpdateUserSchedule,
          usernmae: username,
          schedule: schedule,
        },
      });
    ...
    try {
      ...
      if (users !== null) {
        ...
        this.emit(
          ...
          device,
          username,
          new UpdateUserScheduleError("Error on getting user list through cloud api call", {
            context: { device: deviceSN, username: username, schedule: schedule },
          })
        );
      }
      ...
      const error = ensureError(err);
      rootMainLogger.error(`updateUserSchedule error`, {
        error: getError(error),
        deviceSN: deviceSN,
        username: username,
        schedule: schedule,
  ...
  private onStationDevicePinVerified(deviceSN: string, successfull: boolean): void {
    this.getDevice(deviceSN)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`onStationDevicePinVerified error`, {
          error: getError(error),
          deviceSN: deviceSN,
  ...
  private _emitStationImageDownload(station: Station, file: string, picture: Picture): void {
    ...
    this.getDevicesFromStation(station.getSerial())
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`onStationImageDownload - Set picture error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          file: file,
        });
        ...
        if (
          ...
          this.getDevice(element.device_sn)
            ...
            .catch((err) => {
              const error = ensureError(err);
              if (!(error instanceof DeviceNotFoundError)) {
                rootMainLogger.error("onStationDatabaseQueryLatest Error", {
                  error: getError(error),
                  stationSN: station.getSerial(),
  ...
  private onStationSensorStatus(station: Station, channel: number, status: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station sensor status error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
  ...
  private onStationGarageDoorStatus(station: Station, channel: number, doorId: number, status: number): void {
    this.getStationDevice(station.getSerial(), channel)
      ...
      .catch((err) => {
        const error = ensureError(err);
        rootMainLogger.error(`Station garage door status error`, {
          error: getError(error),
          stationSN: station.getSerial(),
          channel: channel,
        });
