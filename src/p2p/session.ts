...
import {
  ...
  buildCheckCamPayload2,
  buildLookupWithKeyPayload3,
  decodeBase64,
  getVideoCodec,
  checkT8420,
...
import {
  P2PDataMessage,
  P2PDataMessageAudio,
  P2PDataMessageBuilder,
  P2PMessageState,
  P2PDataMessageVideo,
  P2PMessage,
  P2PDataHeader,
  P2PDataMessageState,
  P2PClientProtocolEvents,
  DeviceSerial,
  P2PQueueMessage,
  P2PCommand,
...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
  ...
  private messageStates: SortedMap<number, P2PMessageState> = new SortedMap<number, P2PMessageState>(
    (a: number, b: number) => a - b
  );
  private messageVideoStates: SortedMap<number, P2PVideoMessageState> = new SortedMap<number, P2PVideoMessageState>(
    (a: number, b: number) => a - b
  );
  ...
  private _incrementSequence(sequence: number): number {
    if (sequence < this.MAX_SEQUENCE_NUMBER) return sequence + 1;
    return 0;
  }
  ...
  private _wasSequenceNumberAlreadyProcessed(expectedSequence: number, receivedSequence: number): boolean {
    if (expectedSequence - this.SEQUENCE_PROCESSING_BOUNDARY > 0) {
      ...
      const isInRangeBeforeReset = this._isBetween(
        receivedSequence,
        this.MAX_SEQUENCE_NUMBER + (expectedSequence - this.SEQUENCE_PROCESSING_BOUNDARY),
  ...
  private async sendMessage(errorSubject: string, address: Address, msgID: Buffer, payload?: Buffer): Promise<void> {
    await sendMessage(this.socket, address, msgID, payload).catch((err) => {
      ...
      rootP2PLogger.error(`${errorSubject} - Error`, {
  ...
  private closeEnergySavingDevice(): void {
    if (
      ...
      if (this.esdDisconnectTimeout === undefined) {
        rootP2PLogger.debug(
          `Energy saving device - No more p2p commands to execute or running streams, initiate disconnect timeout in ${this.ESD_DISCONNECT_TIMEOUT} milliseconds...`,
          { stationSN: this.rawStation.station_sn }
        );
  ...
  private async renewDSKKey(): Promise<void> {
    if (this.dskKey === "" || (this.dskExpiration && new Date().getTime() >= this.dskExpiration.getTime())) {
      rootP2PLogger.debug(`DSK keys not present or expired, get/renew it`, {
        stationSN: this.rawStation.station_sn,
        dskKey: this.dskKey,
  ...
  private async cloudLookupByAddressWithTurnServer(
    address: Address,
    origAddress: Address,
    data: Buffer
  ): Promise<void> {
  ...
  private _startConnectTimeout(): void {
    if (this.connectTimeout === undefined)
      this.connectTimeout = setTimeout(() => {
        rootP2PLogger.warn(
          `Tried all hosts, no connection could be established to station ${this.rawStation.station_sn}.`
        );
        this._disconnected();
  ...
  private _connect(address: Address, p2p_did: string): void {
    rootP2PLogger.debug(`Connecting to host ${address.host} on port ${address.port} (CHECK_CAM)`, {
      stationSN: this.rawStation.station_sn,
      address: address,
      p2pDid: p2p_did,
    });
    ...
    for (let i = address.port - 3; i < address.port; i++) this.sendCamCheck({ host: address.host, port: i }, p2p_did);
  ...
  private lookup(host?: string): void {
    ...
    this.lookupTimeout = setTimeout(() => {
      ...
      if (this.localIPAddress !== undefined) this.localIPAddress = undefined;
      this._disconnected();
  ...
  public async sendPing(address: Address): Promise<void> {
    if (
      (this.lastPong && (new Date().getTime() - this.lastPong) / this.getHeartbeatInterval() >= this.MAX_RETRIES) ||
      (this.connectTime &&
        !this.lastPong &&
      ...
      if (!this.energySavingDevice)
        rootP2PLogger.warn(
          `Heartbeat check failed for station ${this.rawStation.station_sn}. Connection seems lost. Try to reconnect...`
        );
      this._disconnected();
    }
  ...
  public sendCommandWithStringPayload(p2pcommand: P2PCommand, customData?: CustomData): void {
    ...
    if (p2pcommand.commandType == CommandType.CMD_SET_PAYLOAD) {
      try {
        ...
        const error = ensureError(err);
        rootP2PLogger.error(`sendCommandWithStringPayload CMD_SET_PAYLOAD - Error`, {
          error: getError(error),
          stationSN: this.rawStation.station_sn,
      ...
      try {
        ...
        const error = ensureError(err);
        rootP2PLogger.error(`sendCommandWithStringPayload CMD_DOORBELL_SET_PAYLOAD - Error`, {
          error: getError(error),
          stationSN: this.rawStation.station_sn,
          p2pcommand: p2pcommand,
          customData: customData,
    ...
    this.sendCommand(
      p2pcommand,
      InternalP2PCommandType.WithStringPayload,
      nested_commandType,
      customData,
      nested_commandType2
    );
  ...
  public sendCommandWithString(p2pcommand: P2PCommand, customData?: CustomData): void {
    if (p2pcommand.channel === undefined) p2pcommand.channel = this.channel;
    if (p2pcommand.strValue === undefined) throw new TypeError("strValue must be defined");
    if (p2pcommand.strValueSub === undefined) throw new TypeError("strValueSub must be defined");
      ...
      if (this.connected) {
        ...
        while ((queuedMessage = this.sendQueue.shift()!) !== undefined) {
          ...
          this.messageStates.forEach((stateMessage) => {
            if (
              stateMessage.commandType === queuedMessage.p2pCommand.commandType &&
        ...
        rootP2PLogger.debug(`Initiate station p2p connection to send queued data`, {
          stationSN: this.rawStation.station_sn,
          queuedDataCount: this.sendQueue.filter(
            (queue) =>
              queue.p2pCommand.commandType !== CommandType.CMD_PING &&
              queue.p2pCommand.commandType !== CommandType.CMD_GET_DEVICE_PING
          ).length,
      ...
      messageState.retryTimeout = setTimeout(() => {
        if (this.connectAddress) {
          sendMessage(this.socket, this.connectAddress, RequestMessageType.DATA, messageState.data).catch((err) => {
            const error = ensureError(err);
            rootP2PLogger.error(`resendNotAcknowledgedCommand - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              sequence: sequence,
            });
  ...
  private async _sendCommand(message: P2PMessageState | P2PQueueMessage): Promise<void> {
    if (isP2PQueueMessage(message)) {
      ...
      if (ageing <= this.MAX_COMMAND_QUEUE_TIMEOUT) {
        ...
        switch (message.p2pCommandType) {
          case InternalP2PCommandType.WithInt:
            payload = buildIntCommandPayload(
              this.encryption,
              this.p2pKey,
              ...
              message.p2pCommand.strValue === undefined ? "" : message.p2pCommand.strValue,
              channel
            );
            break;
          case InternalP2PCommandType.WithIntString:
            payload = buildIntStringCommandPayload(
              this.encryption,
              this.p2pKey,
              this.rawStation.station_sn,
              this.rawStation.p2p_did,
              message.p2pCommand.commandType,
              message.p2pCommand.value as number,
              message.p2pCommand.valueSub === undefined ? 0 : message.p2pCommand.valueSub,
              message.p2pCommand.strValue === undefined ? "" : message.p2pCommand.strValue,
              message.p2pCommand.strValueSub === undefined ? "" : message.p2pCommand.strValueSub,
              channel
            );
            //TODO: Check if this "if" can be moved elsewhere
            if (message.p2pCommand.commandType === CommandType.CMD_NAS_TEST) {
              this.currentMessageState[P2PDataType.DATA].rtspStream[channel] =
                message.p2pCommand.value === 1 ? true : false;
            }
            break;
          case InternalP2PCommandType.WithString:
            payload = buildStringTypeCommandPayload(
              this.encryption,
              this.p2pKey,
              this.rawStation.station_sn,
              this.rawStation.p2p_did,
              message.p2pCommand.commandType,
              message.p2pCommand.strValue as string,
              message.p2pCommand.strValueSub as string,
              channel
            );
            break;
          case InternalP2PCommandType.WithStringPayload:
            payload = buildCommandWithStringTypePayload(
              this.encryption,
              this.p2pKey,
              this.rawStation.station_sn,
              this.rawStation.p2p_did,
              message.p2pCommand.commandType,
              message.p2pCommand.value as string,
              channel
            );
            break;
          default:
        ...
        rootP2PLogger.warn(`Command aged out from send queue for station ${this.rawStation.station_sn}`, {
          commandType: message.p2pCommand.commandType,
          nestedCommandType: message.nestedCommandType,
      ...
      if (message.retries < this.MAX_RETRIES && message.returnCode !== ErrorCode.ERROR_CONNECT_TIMEOUT) {
        ...
        rootP2PLogger.error(`Max p2p command send retries reached.`, {
          stationSN: this.rawStation.station_sn,
        ...
        if (
          message.commandType === CommandType.CMD_START_REALTIME_MEDIA ||
          (message.nestedCommandType === CommandType.CMD_START_REALTIME_MEDIA &&
            message.commandType === CommandType.CMD_SET_PAYLOAD) ||
          message.commandType === CommandType.CMD_RECORD_VIEW ||
          (message.nestedCommandType === ParamType.COMMAND_START_LIVESTREAM &&
            message.commandType === CommandType.CMD_DOORBELL_SET_PAYLOAD)
        ) {
          if (
            this.currentMessageState[P2PDataType.VIDEO].p2pStreaming &&
            message.channel === this.currentMessageState[P2PDataType.VIDEO].p2pStreamChannel
          ) {
            this.endStream(P2PDataType.VIDEO);
          }
          ...
          if (
            ...
            message.channel === this.currentMessageState[P2PDataType.BINARY].p2pStreamChannel
          ) {
            this.endStream(P2PDataType.BINARY);
          }
        }
    ...
    if (messageState.commandType !== CommandType.CMD_PING) {
      ...
      rootP2PLogger.trace(`Added sequence number mapping`, {
        stationSN: this.rawStation.station_sn,
        commandType: message.commandType,
    ...
    if (messageState.retries === 0) {
      if (
        messageState.commandType === CommandType.CMD_START_REALTIME_MEDIA ||
        (messageState.nestedCommandType === CommandType.CMD_START_REALTIME_MEDIA &&
          messageState.commandType === CommandType.CMD_SET_PAYLOAD) ||
        messageState.commandType === CommandType.CMD_RECORD_VIEW ||
        (messageState.nestedCommandType === ParamType.COMMAND_START_LIVESTREAM &&
          messageState.commandType === CommandType.CMD_DOORBELL_SET_PAYLOAD)
      ) {
        if (
          this.currentMessageState[P2PDataType.VIDEO].p2pStreaming &&
          messageState.channel !== this.currentMessageState[P2PDataType.VIDEO].p2pStreamChannel
        ) {
          this.endStream(P2PDataType.VIDEO);
        }
      ...
      } else if (messageState.commandType === CommandType.CMD_STOP_REALTIME_MEDIA) {
    ...
    if (hasHeader(msg, ResponseMessageType.LOCAL_LOOKUP_RESP)) {
      if (!this.connected) {
        ...
        const p2pDid = `${msg
          ...
          .toString("utf8")
          .replace(/[\0]+$/g, "")}`;
        rootP2PLogger.trace(`Received message - LOCAL_LOOKUP_RESP - Got response`, {
          stationSN: this.rawStation.station_sn,
          ip: rinfo.address,
          port: rinfo.port,
          p2pDid: p2pDid,
      ...
      if (!this.connected) {
        ...
        rootP2PLogger.trace(`Received message - LOOKUP_ADDR - Got response`, {
          stationSN: this.rawStation.station_sn,
        ...
        if (ip === "0.0.0.0") {
          rootP2PLogger.trace(`Received message - LOOKUP_ADDR - Got invalid ip address 0.0.0.0, ignoring response...`, {
            stationSN: this.rawStation.station_sn,
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
            response: { ip: ip, port: port },
        ...
        if (isPrivateIp(ip)) this.localIPAddress = ip;

        if (this.connectionType === P2PConnectionType.ONLY_LOCAL) {
          if (isPrivateIp(ip)) {
            this._clearLookupTimeout();
            this._clearLookupRetryTimeout();
            rootP2PLogger.debug(`Trying to connect in ONLY_LOCAL mode...`, {
              stationSN: this.rawStation.station_sn,
              ip: ip,
              port: port,
            });
          ...
          this._clearLookupTimeout();
          this._clearLookupRetryTimeout();
          rootP2PLogger.debug(`Trying to connect in QUICKEST mode...`, {
            stationSN: this.rawStation.station_sn,
            ip: ip,
            port: port,
          });
      ...
      if (!this.connected) {
        rootP2PLogger.debug(
          `Received message - CAM_ID - Connected to station ${this.rawStation.station_sn} on host ${rinfo.address} port ${rinfo.port}`
        );
        this._clearLocalLookupRetryTimeout();
        this._clearLookupRetryTimeout();
        this._clearLookup2RetryTimeout();
        ...
        if (isPrivateIp(rinfo.address)) this.localIPAddress = rinfo.address;
        ...
        if (Device.isSmartSafe(this.rawStation.device_type)) {
          ...
          this.sendMessage(
            `Send smartsafe gateway command to station`,
            this.connectAddress!,
            RequestMessageType.DATA,
            data
          );
          ...
          Device.isLockWifiR10(this.rawStation.device_type) ||
          Device.isLockWifiR20(this.rawStation.device_type)
        ) {
          const tmpSendQueue: Array<P2PQueueMessage> = [...this.sendQueue];
          this.sendQueue = [];
          const payload = buildVoidCommandPayload(Station.CHANNEL);
          const data = Buffer.concat([
            buildCommandHeader(0, CommandType.CMD_GATEWAYINFO),
            payload.subarray(0, payload.length - 2),
            buildCommandHeader(0, CommandType.CMD_PING).subarray(2),
            payload,
          ]);
          const message: P2PMessageState = {
            sequence: this.seqNumber,
          ...
          this.seqNumber = this._incrementSequence(this.seqNumber);
          this.sendMessage(
            `Send lock wifi gateway v12 command to station`,
            this.connectAddress!,
            RequestMessageType.DATA,
            data
          );
          ...
          this.rawStation.devices !== undefined &&
          ...
          this.seqNumber = this._incrementSequence(this.seqNumber);
          this.sendMessage(
            `Send lock wifi gateway command to station`,
            this.connectAddress!,
            RequestMessageType.DATA,
            data
          );
      ...
      if (msg.length > 4) this.lastPongData = msg.subarray(4);
      else this.lastPongData = undefined;
      ...
      rootP2PLogger.debug(`Received message - END`, {
        stationSN: this.rawStation.station_sn,
        remoteAddress: rinfo.address,
        remotePort: rinfo.port,
      });
      ...
      for (let i = 1; i <= numAcks; i++) {
        ...
        rootP2PLogger.trace(`Received message - ACK ${P2PDataType[dataType]} - sequence ${ackedSeqNo}`, {
          stationSN: this.rawStation.station_sn,
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
        ...
        if (dataType === P2PDataType.DATA) {
          ...
          if (msg_state && !msg_state.acknowledged) {
            ...
            if (
              msg_state.commandType === CommandType.CMD_PING ||
              msg_state.commandType === CommandType.CMD_GET_DEVICE_PING
            ) {
              ...
              msg_state.timeout = setTimeout(
                () => {
                  ...
                  if (msg_state.commandType !== CommandType.CMD_GATEWAYINFO) {
                    rootP2PLogger.warn(`Result data for command not received`, {
                      stationSN: this.rawStation.station_sn,
                      message: {
                    ...
                    this.emit("command", {
                      command_type:
                        msg_state.nestedCommandType !== undefined ? msg_state.nestedCommandType : msg_state.commandType,
                      channel: msg_state.channel,
                      return_code: ErrorCode.ERROR_COMMAND_TIMEOUT,
                ...
                msg_state.commandType !== CommandType.CMD_GATEWAYINFO
      ...
      if (this.connected) {
        ...
        rootP2PLogger.debug(
          `Received message - DATA ${P2PDataType[message.type]} - Processing sequence ${message.seqNo}...`,
          {
            stationSN: this.rawStation.station_sn,
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
        ...
        if (message.seqNo === this.expectedSeqNo[dataType]) {
          ...
          rootP2PLogger.trace(`Received message - DATA ${P2PDataType[message.type]} - Received expected sequence`, {
            stationSN: this.rawStation.station_sn,
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
            dataType: P2PDataType[message.type],
          ...
          while (queuedMessage) {
            rootP2PLogger.trace(`Received message - DATA ${P2PDataType[queuedMessage.type]} - Work off queued data`, {
              stationSN: this.rawStation.station_sn,
              remoteAddress: rinfo.address,
              remotePort: rinfo.port,
              dataType: P2PDataType[message.type],
          ...
          rootP2PLogger.trace(
            `Received message - DATA ${P2PDataType[message.type]} - Received already processed sequence`,
            {
              stationSN: this.rawStation.station_sn,
              remoteAddress: rinfo.address,
              remotePort: rinfo.port,
          ...
          if (!this.currentMessageState[dataType].queuedData.get(message.seqNo)) {
            ...
            rootP2PLogger.trace(
              `Received message - DATA ${P2PDataType[message.type]} - Received not expected sequence, added to the queue for future processing`,
              {
                stationSN: this.rawStation.station_sn,
                remoteAddress: rinfo.address,
                remotePort: rinfo.port,
            ...
            rootP2PLogger.trace(
              `Received message - DATA ${P2PDataType[message.type]} - Received not expected sequence, discarded since already present in queue for future processing`,
              {
                stationSN: this.rawStation.station_sn,
                remoteAddress: rinfo.address,
                remotePort: rinfo.port,
      ...
      if (!this.connected) {
        ...
        rootP2PLogger.trace(`Received message - LOOKUP_ADDR2 - Got response`, {
          stationSN: this.rawStation.station_sn,
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
          response: { ip: ip, port: port, data: data.toString("hex") },
        });
        rootP2PLogger.debug(`Connecting to host ${ip} on port ${port} (CHECK_CAM2)...`, {
          stationSN: this.rawStation.station_sn,
          ip: ip,
          port: port,
        ...
        for (let i = 0; i < 4; i++) this.sendCamCheck2({ host: ip, port: port }, data);
      ...
      if (!this.connected && !this.p2pTurnConfirmed) {
        rootP2PLogger.trace(`Received message - TURN_SERVER_OK - Got response`, {
          stationSN: this.rawStation.station_sn,
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
        ...
        this.sendMessage(
          `Send TURN_CLIENT_OK`,
          { host: rinfo.address, port: rinfo.port },
          RequestMessageType.TURN_CLIENT_OK
        );
      ...
      if (!this.connected) {
        ...
        for (let i = 0; i < 4; i++) this.sendCamCheck2({ host: rinfo.address, port: port }, binaryIP);

        rootP2PLogger.trace(`Received message - TURN_SERVER_TOKEN - Got response`, {
          stationSN: this.rawStation.station_sn,
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
      ...
      hasHeader(msg, ResponseMessageType.TURN_SERVER_LOOKUP_OK) ||
      hasHeader(msg, ResponseMessageType.UNKNOWN_83) ||
      hasHeader(msg, ResponseMessageType.TURN_SERVER_LIST)
    ) {
      // Do nothing / ignore
      ...
      if (!this.connected) {
        ...
        rootP2PLogger.trace(`Received message - LOOKUP_RESP - Got response`, {
          stationSN: this.rawStation.station_sn,
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
      ...
      rootP2PLogger.debug(`Received unknown message`, {
        stationSN: this.rawStation.station_sn,
        remoteAddress: rinfo.address,
        remotePort: rinfo.port,
        response: { message: msg.toString("hex"), length: msg.length },
  ...
  private parseDataMessage(message: P2PMessage): void {
    if (
      ...
      rootP2PLogger.trace(`Parsing message - DATA ${P2PDataType[message.type]} - Stream not started ignore this data`, {
        stationSN: this.rawStation.station_sn,
        seqNo: message.seqNo,
      ...
      do {
        ...
        if (firstPartMessage) {
          ...
          if (
            ...
            rootP2PLogger.debug(
              `Parsing message - DATA ${P2PDataType[message.type]} - Discarding unexpected data (infinite loop prevention)`,
              {
                stationSN: this.rawStation.station_sn,
                seqNo: message.seqNo,
        ...
        rootP2PLogger.trace(`Parsing message - DATA ${P2PDataType[message.type]} - Received data`, {
          stationSN: this.rawStation.station_sn,
          seqNo: message.seqNo,
          header: this.currentMessageBuilder[message.type].header,
          bytesRead: this.currentMessageBuilder[message.type].bytesRead,
        ...
        if (
          ...
          if (data.length > 0 && message.type === P2PDataType.DATA) {
            rootP2PLogger.debug(`Parsing message - DATA ${P2PDataType[message.type]} - Parsed data`, {
              stationSN: this.rawStation.station_sn,
              seqNo: message.seqNo,
      ...
      if (runaway_limit >= this.LOOP_RUNAWAY_LIMIT) {
        rootP2PLogger.warn(
          `Infinite loop detected (limit >= ${this.LOOP_RUNAWAY_LIMIT}) during parsing of p2p message`,
          {
            stationSN: this.rawStation.station_sn,
            seqNo: message.seqNo,
            dataType: P2PDataType[message.type],
  ...
  private handleData(message: P2PDataMessage): void {
    if (message.dataType === P2PDataType.CONTROL) {
      ...
      if (result_msg) {
        ...
        rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Received data`, {
          stationSN: this.rawStation.station_sn,
          seqNo: message.seqNo,
        ...
        if (goodSeqNumber) {
          ...
          rootP2PLogger.trace(
            `Handle DATA ${P2PDataType[message.dataType]} - Result data received - Detecting correct sequence number`,
            {
              stationSN: this.rawStation.station_sn,
              commandIdName: commandStr,
              commandId: message.commandId,
        ...
        if (msg_state) {
          if (msg_state.commandType === message.commandId) {
            ...
            const command_type =
              msg_state.nestedCommandType !== undefined ? msg_state.nestedCommandType : msg_state.commandType;
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Result data for command received`, {
              stationSN: this.rawStation.station_sn,
              message: {
                sequence: msg_state.sequence,
                commandType: msg_state.commandType,
            ...
            if (return_code === ErrorCode.ERROR_FAILED_TO_REQUEST) {
              ...
              if (
                command_type === CommandType.CMD_SMARTSAFE_SETTINGS ||
                command_type === CommandType.CMD_SET_PAYLOAD_LOCKV12 ||
                command_type === CommandType.CMD_TRANSFER_PAYLOAD
              ) {
                ...
                this.secondaryCommandTimeout = setTimeout(() => {
                  rootP2PLogger.warn(
                    `Handle DATA ${P2PDataType[message.dataType]} - Result data for secondary command not received`,
                    {
                      stationSN: this.rawStation.station_sn,
                      message: {
                        sequence: msg_state!.sequence,
                  ...
                  this.emit("secondary command", {
                    command_type:
                      msg_state!.nestedCommandType !== undefined
                        ? msg_state!.nestedCommandType
                        : msg_state!.commandType,
                    channel: msg_state!.channel,
                    return_code: ErrorCode.ERROR_COMMAND_TIMEOUT,
              ...
              if (
                msg_state.commandType === CommandType.CMD_START_REALTIME_MEDIA ||
                (msg_state.nestedCommandType === CommandType.CMD_START_REALTIME_MEDIA &&
                ...
                (msg_state.commandType === CommandType.CMD_DOORBELL_SET_PAYLOAD &&
                  msg_state.nestedCommandType === IndoorSoloSmartdropCommandType.CMD_START_SPEAK)
              ) {
                if (return_code === ErrorCode.ERROR_PPCS_SUCCESSFUL) {
                  ...
                  this.emit(
                    "talkback error",
                    msg_state.channel,
                    new TalkbackError("Someone is responding now.", {
                  ...
                  this.emit(
                    "talkback error",
                    msg_state.channel,
                    new TalkbackError("Wait a second, device is busy.", {
                      context: { station: this.rawStation.station_sn, channel: msg_state.channel },
                    })
                  );
                } else {
                  this.emit(
                    "talkback error",
                    msg_state.channel,
                    new TalkbackError("Connect failed please try again later.", {
                      context: { station: this.rawStation.station_sn, channel: msg_state.channel },
                    })
                  );
                }
              } else if (
                msg_state.commandType === CommandType.CMD_STOP_TALKBACK ||
                (msg_state.commandType === CommandType.CMD_DOORBELL_SET_PAYLOAD &&
                  msg_state.nestedCommandType === IndoorSoloSmartdropCommandType.CMD_END_SPEAK)
              ) {
            ...
            this.messageStates.delete(message.seqNo);
            rootP2PLogger.debug(
              `Handle DATA ${P2PDataType[message.dataType]} - Expected different command type for received sequencenumber!`,
              {
                stationSN: this.rawStation.station_sn,
            ...
            rootP2PLogger.warn(
              `P2P protocol instability detected for station ${this.rawStation.station_sn}. Please reinitialise the connection to solve the problem!`
            );
          }
        } else if (
          message.commandId !== CommandType.CMD_PING &&
        ...
        rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Unsupported response`, {
          stationSN: this.rawStation.station_sn,
          message: {
            seqNo: message.seqNo,
      ...
      rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Not implemented data type`, {
        stationSN: this.rawStation.station_sn,
        message: {
          seqNo: message.seqNo,
          commandType: message.commandId,
    ...
    if (!this.currentMessageState[message.dataType].invalidStream) {
      switch (message.commandId) {
        ...
        case CommandType.CMD_AUDIO_FRAME:
          ...
          rootP2PLogger.trace(`Handle DATA ${P2PDataType[message.dataType]} - CMD_AUDIO_FRAME`, {
            stationSN: this.rawStation.station_sn,
            dataSize: message.data.length,
      ...
      rootP2PLogger.debug(
        `Handle DATA ${P2PDataType[message.dataType]} - Invalid stream data, dropping complete stream`,
        {
          stationSN: this.rawStation.station_sn,
          commandIdName: CommandType[message.commandId],
          commandId: message.commandId,
  ...
  private handleDataControl(message: P2PDataMessage): void {
    try {
      ...
      rootP2PLogger.trace(`Handle DATA ${P2PDataType[message.dataType]} - Received data`, {
        stationSN: this.rawStation.station_sn,
        commandIdName: CommandType[message.commandId],
        commandId: message.commandId,
        data: message.data.toString("hex"),
      ...
      switch (message.commandId) {
        ...
        case CommandType.CMD_CAMERA_INFO:
          try {
            ...
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Camera info`, {
              stationSN: this.rawStation.station_sn,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - Camera info - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_CONVERT_MP4_OK:
          ...
          rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_CONVERT_MP4_OK`, {
            stationSN: this.rawStation.station_sn,
            channel: message.channel,
        ...
        case CommandType.CMD_WIFI_CONFIG:
          ...
          rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_WIFI_CONFIG`, {
            stationSN: this.rawStation.station_sn,
            channel: message.channel,
            rssi: rssi,
          });
        ...
        case CommandType.CMD_DOWNLOAD_FINISH:
          rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_DOWNLOAD_FINISH`, {
            stationSN: this.rawStation.station_sn,
            channel: message.channel,
          });
          this.endStream(P2PDataType.BINARY);
        ...
        case CommandType.CMD_DOORBELL_NOTIFY_PAYLOAD:
          try {
            ...
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_DOORBELL_NOTIFY_PAYLOAD`, {
              stationSN: this.rawStation.station_sn,
              payload: str,
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_DOORBELL_NOTIFY_PAYLOAD - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_NAS_SWITCH:
          try {
            const str = getNullTerminatedString(data, "utf8");
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_NAS_SWITCH`, {
              stationSN: this.rawStation.station_sn,
              payload: str,
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_NAS_SWITCH - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.SUB1G_REP_UNPLUG_POWER_LINE:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_UNPLUG_POWER_LINE`, {
              stationSN: this.rawStation.station_sn,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_UNPLUG_POWER_LINE - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.SUB1G_REP_RUNTIME_STATE:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_RUNTIME_STATE`, {
              stationSN: this.rawStation.station_sn,
              payload: data.toString(),
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - SUB1G_REP_RUNTIME_STATE - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_SET_FLOODLIGHT_MANUAL_SWITCH:
          try {
            ...
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_FLOODLIGHT_MANUAL_SWITCH`, {
              stationSN: this.rawStation.station_sn,
              enabled: enabled,
              payload: data.toString(),
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(
              `Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_FLOODLIGHT_MANUAL_SWITCH - Error`,
              {
                error: getError(error),
                stationSN: this.rawStation.station_sn,
                message: {
        ...
        case CommandType.CMD_GET_DEVICE_PING:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DEVICE_PING`, {
              stationSN: this.rawStation.station_sn,
              payload: data.toString(),
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DEVICE_PING - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_NOTIFY_PAYLOAD:
          try {
            rootP2PLogger.debug(`Station Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD`, {
              stationSN: this.rawStation.station_sn,
              payload: data.toString(),
            });
            ...
            if (json !== undefined) {
              if (Device.isLockWifi(this.rawStation.device_type, this.rawStation.station_sn)) {
                ...
                if (
                  json.cmd === CommandType.P2P_ADD_PW ||
                  json.cmd === CommandType.P2P_QUERY_PW ||
                  json.cmd === CommandType.P2P_GET_LOCK_PARAM ||
                  json.cmd === CommandType.P2P_GET_USER_AND_PW_ID
                ) {
                  // encrypted data
                  ...
                  if (aesKey !== undefined) {
                    const decryptedPayload = decryptPayloadData(
                      Buffer.from(json.payload as string, "base64"),
                      Buffer.from(aesKey, "hex"),
                      Buffer.from(getLockVectorBytes(this.rawStation.station_sn), "hex")
                    ).toString();
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock - Received`,
                      {
                        stationSN: this.rawStation.station_sn,
                ...
                if (
                  Device.isLockWifi(this.rawStation.devices[0]?.device_type, this.rawStation.devices[0]?.device_sn) ||
                  Device.isLockWifiNoFinger(this.rawStation.devices[0]?.device_type)
                ) {
                  ...
                  if (payload.seq_num !== undefined) {
                    ...
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD - Lock sequence number`,
                      { stationSN: this.rawStation.station_sn, lockSeqNumber: this.lockSeqNumber, payload: payload }
                    );
                  }
                  ...
                  if (payload.lock_cmd > 0) {
                    if (
                      Device.isLockWifiR10(this.rawStation.devices[0]?.device_type) ||
                      Device.isLockWifiR20(this.rawStation.devices[0]?.device_type)
                    ) {
                      this.emit(
                        "sequence error",
                        message.channel,
                        ESLCommand[ESLBleCommand[payload.lock_cmd] as unknown as number] as unknown as number,
                        payload.seq_num,
                        payload.dev_sn
                      );
                    } else if (
                      Device.isLockWifiT8506(this.rawStation.devices[0]?.device_type) ||
                      Device.isLockWifiT8502(this.rawStation.devices[0]?.device_type) ||
                      ...
                      this.emit(
                        ...
                        message.channel,
                        SmartLockCommand[
                          payload.bus_type! == SmartLockFunctionType.TYPE_2
                            ? (SmartLockBleCommandFunctionType2[payload.lock_cmd] as unknown as number)
                            : (SmartLockBleCommandFunctionType1[payload.lock_cmd] as unknown as number)
                ...
                if (this.deviceSNs[message.channel] !== undefined) {
                  if (payload.lock_payload !== undefined) {
                    ...
                    const key = generateBasicLockAESKey(
                      this.deviceSNs[message.channel].adminUserId,
                      this.rawStation.station_sn
                    );
                    const iv = getLockVectorBytes(this.rawStation.station_sn);

                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_DOORLOCK_DATA_PASS_THROUGH`,
                      {
                        stationSN: this.rawStation.station_sn,
                        commandIdName: CommandType[json.cmd],
                        commandId: json.cmd,
                    ...
                    switch (payload.lock_cmd) {
                      case ESLBleCommand.NOTIFY:
                        ...
                        this.emit(
                          ...
                          notifyBuffer.subarray(3, 4).readInt8().toString()
                        );
                        this.emit(
                          "parameter",
                          message.channel,
                          CommandType.CMD_DOORLOCK_GET_STATE,
                          notifyBuffer.subarray(6, 7).readInt8().toString()
                        );
                        break;
                      default:
                        rootP2PLogger.debug(
                          `Handle DATA ${P2PDataType[message.dataType]} - CMD_DOORLOCK_DATA_PASS_THROUGH - Not implemented`,
                          { stationSN: this.rawStation.station_sn, message: data.toString() }
                        );
                        break;
                    }
                  }
                ...
                if (payload.lock_payload !== undefined) {
                  ...
                  if (fac.getCommandCode() !== ESLBleCommand.NOTIFY) {
                    ...
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock V12 - Received`,
                      { stationSN: this.rawStation.station_sn, fac: fac.toString(), aesKey: aesKey }
                    );
                    ...
                    try {
                      ...
                      const commandType = Number.parseInt(
                        ESLCommand[ESLBleCommand[fac.getCommandCode()!] as unknown as number]
                      );
                      const customData = {
                      ...
                      rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock V12 return code: ${returnCode}`,
                        {
                          stationSN: this.rawStation.station_sn,
                          commandIdName: CommandType[json.cmd],
                          commandId: json.cmd,
                          decoded: data.toString("hex"),
                          bleCommandCode: ESLBleCommand[fac.getCommandCode()!],
                      ...
                      switch (fac.getCommandCode()) {
                        ...
                        case ESLBleCommand.NOTIFY:
                          this.emit(
                            ...
                            message.channel,
                            CommandType.CMD_SMARTLOCK_QUERY_BATTERY_LEVEL,
                            parsePayload.readInt8(BleParameterIndex.ONE).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_QUERY_STATUS,
                            parsePayload.readInt8(BleParameterIndex.TWO).toString()
                          );
                          break;
                        case ESLBleCommand.GET_LOCK_PARAM:
                          ...
                          this.emit(
                            ...
                            parsePayload.readUint16LE(BleParameterIndex.THREE).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE,
                            parsePayload.readInt8(BleParameterIndex.FOUR).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE_STARTTIME,
                            parsePayload.readStringHex(BleParameterIndex.FIVE)
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE_ENDTIME,
                            parsePayload.readStringHex(BleParameterIndex.SIX)
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_ONE_TOUCH_LOCK,
                            parsePayload.readInt8(BleParameterIndex.SEVEN).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_WRONG_TRY_PROTECT,
                            parsePayload.readInt8(BleParameterIndex.EIGHT).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_WRONG_TRY_LOCKDOWN,
                            parsePayload.readUint16LE(BleParameterIndex.NINE).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_WRONG_TRY_ATTEMPTS,
                            parsePayload.readInt8(BleParameterIndex.TEN).toString()
                          );
                          this.emit(
                            "parameter",
                            message.channel,
                            CommandType.CMD_SMARTLOCK_SCRAMBLE_PASSCODE,
                            parsePayload.readInt8(BleParameterIndex.ELEVEN).toString()
                          );
                        ...
                        case ESLBleCommand.ADD_PW:
                          if (
                            ...
                            customData.customData.command.name === CommandName.DeviceAddUser
                          ) {
                            this.api.updateUserPassword(
                              customData.customData.command.value?.deviceSN,
                              customData.customData.command.value?.shortUserId,
                              parsePayload.readStringHex(BleParameterIndex.ONE),
                        ...
                        default:
                          rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock - Not implemented`,
                            {
                              stationSN: this.rawStation.station_sn,
                              fac: fac.toString(),
                              returnCode: returnCode,
                      ...
                      const error = ensureError(err);
                      rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock V12 unable to decrypt (maybe event from other lock connection)`,
                        {
                          error: getError(error),
                          stationSN: this.rawStation.station_sn,
                          commandIdName: CommandType[json.cmd],
                    ...
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock V12 - Received notify`,
                      { stationSN: this.rawStation.station_sn, fac: fac.toString() }
                    );
                  }
                } else {
                  rootP2PLogger.debug(
                    `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Lock V12 - Unexpected response`,
                    {
                      stationSN: this.rawStation.station_sn,
                      commandIdName: CommandType[json.cmd],
                      commandId: json.cmd,
                ...
                if (payload.lock_payload !== undefined) {
                  try {
                    ...
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock - Received`,
                      { stationSN: this.rawStation.station_sn, fac: fac.toString() }
                    );
                    if (!fac.isPartial()) {
                      let data = fac.getData();
                      if (data) {
                        if (fac.isEncrypted()) {
                          const aesKey = generateSmartLockAESKey(
                            this.rawStation.member.admin_user_id,
                            Number.parseInt(payload.time, 16)
                          );
                        ...
                        if (data.length > 0 && fac.getDataType() !== undefined) {
                          const returnCode = data.readInt8(0);
                          const functionTypeCommand =
                            fac.getDataType() === SmartLockFunctionType.TYPE_2
                              ? SmartLockBleCommandFunctionType2
                              : SmartLockBleCommandFunctionType1;
                          const commandType = Number.parseInt(
                            SmartLockCommand[functionTypeCommand[fac.getCommandCode()!] as unknown as number]
                          );
                          const customData = {
                            ...this.customDataStaging[commandType],
                          };
                          ...
                          rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock  return code: ${returnCode}`,
                            {
                              stationSN: this.rawStation.station_sn,
                              commandIdName: CommandType[json.cmd],
                              commandId: json.cmd,
                            ...
                            switch (fac.getCommandCode()) {
                              ...
                              case SmartLockBleCommandFunctionType2.GET_LOCK_PARAM:
                                ...
                                this.emit(
                                  ...
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_AUTO_LOCK_TIMER,
                                  parsePayload.readUint16LE(BleParameterIndex.TWO).toString()
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE,
                                  parsePayload.readInt8(BleParameterIndex.THREE).toString()
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_AUTO_LOCK_SCHEDULE_STARTTIME,
                                  parsePayload.readStringHex(BleParameterIndex.FOUR)
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                                ...
                                this.emit(
                                  ...
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_ONE_TOUCH_LOCK,
                                  parsePayload.readInt8(BleParameterIndex.SIX).toString()
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                                ...
                                this.emit(
                                  ...
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_WRONG_TRY_ATTEMPTS,
                                  parsePayload.readInt8(BleParameterIndex.NINE).toString()
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                                  CommandType.CMD_SMARTLOCK_WRONG_TRY_LOCKDOWN,
                                  parsePayload.readUint16LE(BleParameterIndex.TEN).toString()
                                );
                                this.emit(
                                  "parameter",
                                  message.channel,
                          ...
                          rootP2PLogger.debug(
                            `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock - Unsupported data length or missint function type`,
                            { stationSN: this.rawStation.station_sn, fac: fac.toString() }
                          );
                        }
                      } else {
                        rootP2PLogger.debug(
                          `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock - No data`,
                          { stationSN: this.rawStation.station_sn, fac: fac.toString() }
                        );
                      }
                    } else {
                      rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock - Partial data`,
                        { stationSN: this.rawStation.station_sn, fac: fac.toString() }
                      );
                    }
                  } catch (err) {
                    const error = ensureError(err);
                    rootP2PLogger.error(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock Error`,
                      {
                        error: getError(error),
                        stationSN: this.rawStation.station_sn,
                        commandIdName: CommandType[json.cmd],
                  ...
                  rootP2PLogger.debug(
                    `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Smart Lock  - Unexpected response`,
                    {
                      stationSN: this.rawStation.station_sn,
                      commandIdName: CommandType[json.cmd],
                      commandId: json.cmd,
                ...
                rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe`, {
                  stationSN: this.rawStation.station_sn,
                  commandIdName: CommandType[json.cmd],
                  commandId: json.cmd,
                });
                switch (json.cmd) {
                  case CommandType.CMD_SMARTSAFE_SETTINGS: {
                    ...
                    try {
                      ...
                      rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe return code: ${data.data.readInt8(0)}`,
                        {
                          stationSN: this.rawStation.station_sn,
                          commandIdName: CommandType[json.cmd],
                          commandId: json.cmd,
                          decoded: data,
                          commandCode: SmartSafeCommandCode[data.commandCode],
                          returnCode: returnCode,
                          channel: customData?.channel,
                          customData: customData?.customData,
                        }
                      ...
                      const error = ensureError(err);
                      rootP2PLogger.error(
                        `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe Error`,
                        {
                          error: getError(error),
                          stationSN: this.rawStation.station_sn,
                          commandIdName: CommandType[json.cmd],
                  ...
                  case CommandType.CMD_SMARTSAFE_STATUS_UPDATE: {
                    ...
                    switch (payload.event_type) {
                      case SmartSafeEvent.LOCK_STATUS: {
                        ...
                        rootP2PLogger.debug(
                          `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe Status update - LOCK_STATUS`,
                          { stationSN: this.rawStation.station_sn, eventValues: eventValues }
                        );
                        /*
                                                    type values:
                                                        1: Unlocked by PIN
                      ...
                      default:
                        rootP2PLogger.debug(
                          `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe Status update - Not implemented`,
                          { stationSN: this.rawStation.station_sn, message: data.toString() }
                        );
                        break;
                    }
                  ...
                  default:
                    rootP2PLogger.debug(
                      `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD SmartSafe - Not implemented`,
                      { stationSN: this.rawStation.station_sn, message: data.toString() }
                    );
                    break;
                }
                ...
                rootP2PLogger.debug(
                  `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD EntrySensor Status update`,
                  { stationSN: this.rawStation.station_sn, status: payload?.status }
                );
                if (payload) {
                ...
                rootP2PLogger.debug(
                  `Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD GarageDoor Status update`,
                  {
                    stationSN: this.rawStation.station_sn,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_NOTIFY_PAYLOAD Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_GET_DELAY_ALARM:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DELAY_ALARM :`, {
              stationSN: this.rawStation.station_sn,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_DELAY_ALARM - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_SET_TONE_FILE:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_TONE_FILE :`, {
              stationSN: this.rawStation.station_sn,
              payload: data.toString("hex"),
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_TONE_FILE - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_SET_SNOOZE_MODE:
          ...
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_SNOOZE_MODE`, {
              stationSN: this.rawStation.station_sn,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_SET_SNOOZE_MODE - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_DATABASE_IMAGE:
          ...
          try {
            const str = getNullTerminatedString(data, "utf8");
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_DATABASE_IMAGE`, {
              stationSN: this.rawStation.station_sn,
              message: str,
            });
        ...
        case CommandType.CMD_GET_TFCARD_STATUS:
          try {
            rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_TFCARD_STATUS :`, {
              stationSN: this.rawStation.station_sn,
              payload: data.toString("hex"),
            });
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GET_TFCARD_STATUS - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_DATABASE:
          try {
            ...
            switch (databaseResponse.cmd) {
                  ...
                  for (const tableRecord of record.payload) {
                    ...
                    if (record.table_name === "history_record_info") {
                      ...
                      tmpRecord.picture.push({
                        ...
                        person_recog_flag: (tableRecord as P2PDatabaseQueryLocalRecordCropPictureInfo)
                          .person_recog_flag,
                      ...
                      rootP2PLogger.debug(
                        `Handle DATA ${P2PDataType[message.dataType]} - Not implemented - CMD_DATABASE_QUERY_LOCAL - table_name: ${record.table_name}`,
                        { stationSN: this.rawStation.station_sn }
                      );
                    }
              ...
              default:
                rootP2PLogger.debug(
                  `Handle DATA ${P2PDataType[message.dataType]} - Not implemented - CMD_DATABASE message`,
                  {
                    stationSN: this.rawStation.station_sn,
                    commandIdName: CommandType[message.commandId],
                    commandId: message.commandId,
            ...
            const error = ensureError(err);
            rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - CMD_DATABASE - Error`, {
              error: getError(error),
              stationSN: this.rawStation.station_sn,
              message: {
                seqNo: message.seqNo,
        ...
        case CommandType.CMD_GATEWAYINFO:
          ...
          rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - CMD_GATEWAYINFO - cipherID`, {
            stationSN: this.rawStation.station_sn,
            channel: message.channel,
            data: data.toString("hex"),
            cipherID: cipherID,
          ...
          this.api
            ...
            .then((cipher) => {
              rootP2PLogger.debug(
                `Handle DATA ${P2PDataType[message.dataType]} - CMD_GATEWAYINFO - get cipher with cipherID`,
                {
                  stationSN: this.rawStation.station_sn,
                  channel: message.channel,
                  data: data.toString("hex"),
            ...
            .catch((err) => {
              ...
              rootP2PLogger.debug(
                `Handle DATA ${P2PDataType[message.dataType]} - CMD_GATEWAYINFO - set encryption level 1 (fallback)`,
                {
                  error: getError(error),
                  stationSN: this.rawStation.station_sn,
                  message: {
          ...
          break;
        default:
          rootP2PLogger.debug(`Handle DATA ${P2PDataType[message.dataType]} - Not implemented`, {
            stationSN: this.rawStation.station_sn,
            commandIdName: CommandType[message.commandId],
            commandId: message.commandId,
            channel: message.channel,
      ...
      const error = ensureError(err);
      rootP2PLogger.error(`Handle DATA ${P2PDataType[message.dataType]} - Error`, {
        error: getError(error),
        stationSN: this.rawStation.station_sn,
        message: {
          seqNo: message.seqNo,
  ...
  private scheduleHeartbeat(): void {
    if (this.isConnected()) {
      ...
      rootP2PLogger.debug(`Heartbeat not activated because no connection is present!`, {
        stationSN: this.rawStation.station_sn,
      });
    }
  }
  ...
  private scheduleP2PKeepalive(): void {
    if (this.isConnected()) {
      ...
      rootP2PLogger.debug(`P2P keepalive not activated because no connection is present`, {
        stationSN: this.rawStation.station_sn,
      });
    }
  }
  ...
  private endStream(datatype: P2PDataType, sendStopCommand = false): void {
    if (this.currentMessageState[datatype].p2pStreaming) {
      ...
      if (this.currentMessageState[datatype].queuedData.size > 0) {
        this.expectedSeqNo[datatype] = this._incrementSequence(
          [...this.currentMessageState[datatype].queuedData.keys()][
            this.currentMessageState[datatype].queuedData.size - 1
          ]
        );
      }
      this.initializeMessageBuilder(datatype);
  ...
  private emitStreamStartEvent(datatype: P2PDataType): void {
    ...
    if (datatype === P2PDataType.VIDEO) {
      this.emit(
        ...
        this.currentMessageState[datatype].audioStream!
      );
  ...
  public isRTSPLiveStreaming(channel: number): boolean {
    return this.currentMessageState[P2PDataType.DATA].rtspStreaming[channel]
  ...
  public updateRawStation(value: StationListResponse): void {
    ...
    if (Device.hasBattery(this.rawStation.device_type)) {
      ...
      if (this.energySavingDevice) {
        rootP2PLogger.debug(
          `Identified standalone battery device ${this.rawStation.station_sn} => activate p2p keepalive command`
        );
      }
    } else {
  ...
  private async _sendVideoData(message: P2PVideoMessageState): Promise<void> {
    if (message.retries < this.MAX_RETRIES) {
      ...
      rootP2PLogger.error(
        `Sending video data - Max send video data retries ${this.messageVideoStates.get(message.sequence)?.retries} reached. Discard data.`,
        {
          stationSN: this.rawStation.station_sn,
          sequence: message.sequence,
          channel: message.channel,
      ...
      this.emit(
        "talkback error",
        message.channel,
        new TalkbackError("Max send video data retries reached. Discard data packet.", {
